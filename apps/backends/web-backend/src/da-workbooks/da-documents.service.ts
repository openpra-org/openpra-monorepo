...
import { DaWorkbookFile, type DaWorkbookDocumentDocument } from "./da-workbook-document.schema";
...
export interface DaDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class DaDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(DaDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(DaWorkbook.name) private readonly daWorkbookModel: Model<DaWorkbookDocument>,
    @InjectModel(DaWorkbookFile.name) private readonly daDocModel: Model<DaWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_DA_DOCUMENTS_BUCKET"] ?? "da-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.daWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("DA workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<DaDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.daDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<DaDocumentEntry> {
    ...
    const created = await this.daDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.daDocModel.findOne({ workbookId, documentId }).exec();
