...
import { EsDocumentsService, type EsDocumentEntry } from "./es-documents.service";
...
@Controller("es-workbooks/:id/documents")
@UseGuards(JwtAuthGuard)
export class EsDocumentsController {
  constructor(private readonly esDocumentsService: EsDocumentsService) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  list(@Param("id") id: string, @Req() req: AuthenticatedRequest): Promise<EsDocumentEntry[]> {
    return this.esDocumentsService.list(id, { username: req.user!.username });
  }
  ...
  async remove(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<void> {
    await this.esDocumentsService.remove(id, documentId, { username: req.user!.username });
  }
  ...
  download(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<{ url: string; filename: string }> {
    return this.esDocumentsService.presignedDownload(id, documentId, { username: req.user!.username });
  }
}
