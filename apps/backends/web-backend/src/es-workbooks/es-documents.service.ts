...
import { EsWorkbookFile, type EsWorkbookDocumentDocument } from "./es-workbook-document.schema";
...
@Injectable()
export class EsDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(EsDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(EsWorkbook.name) private readonly esWorkbookModel: Model<EsWorkbookDocument>,
    @InjectModel(EsWorkbookFile.name) private readonly esDocModel: Model<EsWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_ES_DOCUMENTS_BUCKET"] ?? "es-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.esWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("ES workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<EsDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.esDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<EsDocumentEntry> {
    ...
    const created = await this.esDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.esDocModel.findOne({ workbookId, documentId }).exec();
