...
import { EsqDocumentsService, type EsqDocumentEntry } from "./esq-documents.service";
...
@Controller("esq-workbooks/:id/documents")
@UseGuards(JwtAuthGuard)
export class EsqDocumentsController {
  constructor(private readonly esqDocumentsService: EsqDocumentsService) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  list(@Param("id") id: string, @Req() req: AuthenticatedRequest): Promise<EsqDocumentEntry[]> {
    return this.esqDocumentsService.list(id, { username: req.user!.username });
  }
  ...
  async remove(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<void> {
    await this.esqDocumentsService.remove(id, documentId, { username: req.user!.username });
  }
  ...
  download(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<{ url: string; filename: string }> {
    return this.esqDocumentsService.presignedDownload(id, documentId, { username: req.user!.username });
  }
}
