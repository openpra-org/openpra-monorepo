...
import { EsqWorkbookFile, type EsqWorkbookDocumentDocument } from "./esq-workbook-document.schema";
...
export interface EsqDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class EsqDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(EsqDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(EsqWorkbook.name) private readonly esqWorkbookModel: Model<EsqWorkbookDocument>,
    @InjectModel(EsqWorkbookFile.name) private readonly esqDocModel: Model<EsqWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_ESQ_DOCUMENTS_BUCKET"] ?? "esq-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.esqWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("ESQ workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<EsqDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.esqDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<EsqDocumentEntry> {
    ...
    const created = await this.esqDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.esqDocModel.findOne({ workbookId, documentId }).exec();
