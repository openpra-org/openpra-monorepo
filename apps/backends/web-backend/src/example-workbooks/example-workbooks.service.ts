...
export interface OtherHazardsPraExampleBundle {
  otherHazardsPra: ExampleWorkbookResponse;
  configurationControl: ExampleWorkbookResponse;
  newlyDevelopedMethods: ExampleWorkbookResponse[];
}
...
export class ExampleWorkbooksService implements OnModuleInit {
  ...
  getOtherHazardsPraExamples(): IeExampleOption[] {
    return OTHER_HAZARDS_PRA_EXAMPLES.map((entry) => ({ id: entry.id, label: entry.label }));
  }

  async getOtherHazardsPraBundle(exampleId?: string): Promise<OtherHazardsPraExampleBundle> {
    const entry = OTHER_HAZARDS_PRA_EXAMPLES.find((item) => item.id === exampleId) ?? OTHER_HAZARDS_PRA_EXAMPLES[0];
    const otherHazardsPra = await this.findBySlug(entry.slug);
    const configurationControl = await this.findBySlug(CC_GENERIC_1_SLUG);
    const nmDocs = await this.exampleModel.find({ kind: "NEWLY_DEVELOPED_METHOD" }).sort({ slug: 1 }).exec();
    return { otherHazardsPra, configurationControl, newlyDevelopedMethods: nmDocs.map(toResponse) };
  }
}
