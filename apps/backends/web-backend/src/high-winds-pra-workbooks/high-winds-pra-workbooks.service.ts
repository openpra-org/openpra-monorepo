...
function toResponse(doc: HighWindsPraWorkbookDocument, myRoles: WorkbookRoleName[]): HighWindsPraWorkbookResponse {
  return {
...
@Injectable()
export class HighWindsPraWorkbooksService {
  constructor(
  ...
  private async findOrInitialize(workbookId: string, acting: ActingUser): Promise<HighWindsPraWorkbookDocument> {
    const existing = await this.workbookModel.findOne({ workbookId }).exec();
    if (existing) {
      const current = existing.mef as { name?: string; owner?: string };
      const healed = healMef(existing.mef, createBlankHighWindsPra(current.name ?? "High Winds PRA Workbook", current.owner ?? existing.ownerUsername));
    ...
    const baseWorkbook = await this.baseWorkbookModel.findOne({ _id: workbookId, elementCode: "W" }).exec();
    if (!baseWorkbook) throw new NotFoundException("High Winds PRA workbook not found");
    ...
    try {
      const initialized = await this.workbookModel.create({
        ...
        mef: createBlankHighWindsPra(baseWorkbook.name, baseWorkbook.ownerUsername),
      });
  ...
  async findOne(workbookId: string, acting: ActingUser): Promise<HighWindsPraWorkbookResponse> {
  ...
  async patchMef(workbookId: string, operations: unknown, acting: ActingUser): Promise<HighWindsPraWorkbookResponse> {
    const doc = await this.findOrInitialize(workbookId, acting);
    const { role } = await this.projectsService.resolveAccess(doc.projectId, acting);
    if (role === "viewer") throw new ForbiddenException("You cannot edit this High Winds PRA workbook");
    const parsed = HighWindsPRASchema.safeParse(stripNulls(mergeWorkbookPatch(doc.mef, operations)));
    if (!parsed.success) throw new ForbiddenException(`Invalid High Winds PRA workbook payload: ${parsed.error.message}`);
    doc.mef = synchronizeHighWindsPraDerivedRegisters(parsed.data);
    await doc.save();
    return toResponse(doc, await this.loadMyRoles(workbookId, acting.username));
  }

  async loadExample(workbookId: string, acting: ActingUser, exampleId?: string): Promise<HighWindsPraWorkbookResponse> {
    const doc = await this.findOrInitialize(workbookId, acting);
    await this.projectsService.resolveAccess(doc.projectId, acting);
    ...
    const example = await this.exampleWorkbooksService.getHighWindsPraBundle(exampleId);
    const parsed = HighWindsPRASchema.safeParse(stripNulls(example.highWindsPra.mef));
    ...
    doc.mef = { ...synchronizeHighWindsPraDerivedRegisters(parsed.data), workflowState: "DRAFT", workflowHistory: [{ state: "DRAFT", enteredAt: new Date().toISOString(), actor: acting.username, note: "Loaded from example workbook" }] };
    await doc.save();
    await this.signoffModel.deleteMany({ workbookId }).exec();
    return toResponse(doc, myRoles);
  }

  async unloadExample(workbookId: string, acting: ActingUser): Promise<HighWindsPraWorkbookResponse> {
    const doc = await this.findOrInitialize(workbookId, acting);
    await this.projectsService.resolveAccess(doc.projectId, acting);
    const myRoles = await this.loadMyRoles(workbookId, acting.username);
