...
import { HrDocumentsService, type HrDocumentEntry } from "./hr-documents.service";
...
@Controller("hr-workbooks/:id/documents")
@UseGuards(JwtAuthGuard)
export class HrDocumentsController {
  constructor(private readonly hrDocumentsService: HrDocumentsService) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  list(@Param("id") id: string, @Req() req: AuthenticatedRequest): Promise<HrDocumentEntry[]> {
    return this.hrDocumentsService.list(id, { username: req.user!.username });
  }
  ...
  async remove(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<void> {
    await this.hrDocumentsService.remove(id, documentId, { username: req.user!.username });
  }
  ...
  download(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<{ url: string; filename: string }> {
    return this.hrDocumentsService.presignedDownload(id, documentId, { username: req.user!.username });
  }
}
