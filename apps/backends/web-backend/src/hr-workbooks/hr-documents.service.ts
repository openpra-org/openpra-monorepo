...
import { HrWorkbookFile, type HrWorkbookDocumentDocument } from "./hr-workbook-document.schema";
...
export interface HrDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class HrDocumentsService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectModel(HrWorkbookFile.name) private readonly hrDocModel: Model<HrWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_HR_DOCUMENTS_BUCKET"] ?? "hr-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.hrWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("HR workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<HrDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.hrDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<HrDocumentEntry> {
    ...
    const created = await this.hrDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.hrDocModel.findOne({ workbookId, documentId }).exec();
