...
import { IeWorkbookFile, type IeWorkbookDocumentDocument } from "./ie-workbook-document.schema";
...
export interface IeDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class IeDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(IeDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(IeWorkbook.name) private readonly ieWorkbookModel: Model<IeWorkbookDocument>,
    @InjectModel(IeWorkbookFile.name) private readonly ieDocModel: Model<IeWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_IE_DOCUMENTS_BUCKET"] ?? "ie-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.ieWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("IE workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<IeDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.ieDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<IeDocumentEntry> {
    ...
    const created = await this.ieDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.ieDocModel.findOne({ workbookId, documentId }).exec();
