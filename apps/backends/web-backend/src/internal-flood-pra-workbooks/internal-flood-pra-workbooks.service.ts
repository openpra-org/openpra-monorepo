...
function toResponse(doc: InternalFloodPraWorkbookDocument, myRoles: WorkbookRoleName[]): InternalFloodPraWorkbookResponse {
...
@Injectable()
export class InternalFloodPraWorkbooksService {
  constructor(
    @InjectModel(InternalFloodPraWorkbook.name) private readonly workbookModel: Model<InternalFloodPraWorkbookDocument>,
  ...
  private async findOrInitialize(workbookId: string, acting: ActingUser): Promise<InternalFloodPraWorkbookDocument> {
    const existing = await this.workbookModel.findOne({ workbookId }).exec();
    if (existing) {
      const current = existing.mef as { name?: string; owner?: string };
      const healed = healMef(existing.mef, createBlankInternalFloodPra(current.name ?? "Internal Flood PRA Workbook", current.owner ?? existing.ownerUsername));
      const parsed = InternalFloodPRASchema.safeParse(healed);
      if (parsed.success && JSON.stringify(parsed.data) !== JSON.stringify(existing.mef)) { existing.mef = parsed.data; await existing.save(); }
      return existing;
    }
    const baseWorkbook = await this.baseWorkbookModel.findOne({ _id: workbookId, elementCode: "FL" }).exec();
    if (!baseWorkbook) throw new NotFoundException("Internal Flood PRA workbook not found");
    await this.projectsService.resolveAccess(baseWorkbook.projectId, acting);
    try {
      const initialized = await this.workbookModel.create({ workbookId, projectId: baseWorkbook.projectId, ownerUsername: baseWorkbook.ownerUsername, mef: createBlankInternalFloodPra(baseWorkbook.name, baseWorkbook.ownerUsername) });
  ...
  async loadExample(workbookId: string, acting: ActingUser, exampleId?: string): Promise<InternalFloodPraWorkbookResponse> {
    const doc = await this.findOrInitialize(workbookId, acting); await this.projectsService.resolveAccess(doc.projectId, acting); const myRoles = await this.loadMyRoles(workbookId, acting.username);
    ...
    const example = await this.exampleWorkbooksService.getInternalFloodPraBundle(exampleId);
    const parsed = InternalFloodPRASchema.safeParse(stripNulls(example.internalFloodPra.mef));
    ...
    doc.mef = { ...synchronizeInternalFloodPraDerivedRegisters(parsed.data), workflowState: "DRAFT", workflowHistory: [{ state: "DRAFT", enteredAt: new Date().toISOString(), actor: acting.username, note: "Loaded from example workbook" }] };
    await doc.save(); await this.signoffModel.deleteMany({ workbookId }).exec(); return toResponse(doc, myRoles);
  }
  async unloadExample(workbookId: string, acting: ActingUser): Promise<InternalFloodPraWorkbookResponse> {
    const doc = await this.findOrInitialize(workbookId, acting); await this.projectsService.resolveAccess(doc.projectId, acting); const myRoles = await this.loadMyRoles(workbookId, acting.username);
