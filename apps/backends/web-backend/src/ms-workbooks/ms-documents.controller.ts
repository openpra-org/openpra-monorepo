...
import { MsDocumentsService, type MsDocumentEntry } from "./ms-documents.service";
...
@Controller("ms-workbooks/:id/documents")
@UseGuards(JwtAuthGuard)
export class MsDocumentsController {
  constructor(private readonly msDocumentsService: MsDocumentsService) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  list(@Param("id") id: string, @Req() req: AuthenticatedRequest): Promise<MsDocumentEntry[]> {
    return this.msDocumentsService.list(id, { username: req.user!.username });
  }
  ...
  async remove(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<void> {
    await this.msDocumentsService.remove(id, documentId, { username: req.user!.username });
  }
  ...
  download(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<{ url: string; filename: string }> {
    return this.msDocumentsService.presignedDownload(id, documentId, { username: req.user!.username });
  }
}
