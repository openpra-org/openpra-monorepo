...
import { MsWorkbookFile, type MsWorkbookDocumentDocument } from "./ms-workbook-document.schema";
...
export interface MsDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class MsDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(MsDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(MsWorkbook.name) private readonly msWorkbookModel: Model<MsWorkbookDocument>,
    @InjectModel(MsWorkbookFile.name) private readonly msDocModel: Model<MsWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_MS_DOCUMENTS_BUCKET"] ?? "ms-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.msWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("MS workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<MsDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.msDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<MsDocumentEntry> {
    ...
    const created = await this.msDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.msDocModel.findOne({ workbookId, documentId }).exec();
