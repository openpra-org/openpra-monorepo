...
export class PosWorkbooksService {
  ...
  async unloadExample(workbookId: string, acting: ActingUser): Promise<PosWorkbookResponse> {
    const doc = await this.posWorkbookModel.findOne({ workbookId }).exec();
    if (!doc) throw new NotFoundException("POS workbook not found");
    await this.projectsService.resolveAccess(doc.projectId, acting);
    const myRoles = await this.loadMyRoles(workbookId, acting.username);
    ...
    const template = createBlankPos(restoredObj.name ?? "POS Workbook", restoredObj.owner ?? acting.username);
    const healed = healMef(restored, template);
    const parsed = PlantOperatingStatesAnalysisSchema.safeParse(healed);
