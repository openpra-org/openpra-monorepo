...
import { RcDocumentsService, type RcDocumentEntry } from "./rc-documents.service";
...
@Controller("rc-workbooks/:id/documents")
@UseGuards(JwtAuthGuard)
export class RcDocumentsController {
  constructor(private readonly rcDocumentsService: RcDocumentsService) {}

  @Get()
  @HttpCode(HttpStatus.OK)
  list(@Param("id") id: string, @Req() req: AuthenticatedRequest): Promise<RcDocumentEntry[]> {
    return this.rcDocumentsService.list(id, { username: req.user!.username });
  }
  ...
  async remove(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<void> {
    await this.rcDocumentsService.remove(id, documentId, { username: req.user!.username });
  }
  ...
  download(@Param("id") id: string, @Param("documentId") documentId: string, @Req() req: AuthenticatedRequest): Promise<{ url: string; filename: string }> {
    return this.rcDocumentsService.presignedDownload(id, documentId, { username: req.user!.username });
  }
}
