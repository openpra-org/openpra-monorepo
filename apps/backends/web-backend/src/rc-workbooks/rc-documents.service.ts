...
export class RcDocumentsService implements OnModuleInit {
  ...
  async list(workbookId: string, acting: ActingUser): Promise<RcDocumentEntry[]> {
    ...
    const docs = await this.rcDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<RcDocumentEntry> {
