...
describe("RC weather persistence", () => {
  ...
  const save = (baseRevision: number, settings: object, confirm = false) => request(t.app.getHttpServer()).patch("/rc-workbooks/rc-test/weather").send({ baseRevision, settings, confirm });
  const setup = async () => {
  ...
  it("imports and pages real records, protects original files, and persists the analyst review", async () => {
    ...
    await save(w.revision, w.settings, true).expect(400);
    const saved = (await save(w.revision, { ...w.settings, nearbySite: { latitude: 35.31028, longitude: -93.23194 } }, true).expect(200)).body;
  ...
  it("rejects unauthorized, stale, malformed and forged inputs without losing saved weather", async () => {
    ...
    await save(w.revision, { ...w.settings, year: 2021 }).expect(400);
    await save(w.revision, { ...w.settings, windSectors: 32 }).expect(400);
    ...
    await save(w.revision, w.settings).expect(403);
  });
  ...
  it("allows a fresh review when the site moves onto the weather location", async () => {
    ...
    const reviewed = (await save(w.revision, { ...w.settings, nearbySite: s.settings }, true).expect(200)).body;
    ...
    const next = (await save(reviewed.revision, reviewed.settings, true).expect(200)).body;
