...
import { ScWorkbookFile, type ScWorkbookDocumentDocument } from "./sc-workbook-document.schema";
...
export interface ScDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class ScDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(ScDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(ScWorkbook.name) private readonly scWorkbookModel: Model<ScWorkbookDocument>,
    @InjectModel(ScWorkbookFile.name) private readonly scDocModel: Model<ScWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_SC_DOCUMENTS_BUCKET"] ?? "sc-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.scWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("SC workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<ScDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.scDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<ScDocumentEntry> {
    ...
    const created = await this.scDocModel.create({
      documentId,
      workbookId,
  ...
  async remove(workbookId: string, documentId: string, acting: ActingUser): Promise<void> {
    ...
    const doc = await this.scDocModel.findOne({ workbookId, documentId }).exec();
