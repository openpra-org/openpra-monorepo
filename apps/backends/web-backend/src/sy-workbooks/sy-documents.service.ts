...
import { SyWorkbookFile, type SyWorkbookDocumentDocument } from "./sy-workbook-document.schema";
...
export interface SyDocumentEntry {
  documentId: string;
  filename: string;
  mimeType: string;
  size: number;
...
@Injectable()
export class SyDocumentsService implements OnModuleInit {
  private readonly logger = new Logger(SyDocumentsService.name);
  private client!: Client;
  private bucket!: string;

  constructor(
    @InjectModel(SyWorkbook.name) private readonly syWorkbookModel: Model<SyWorkbookDocument>,
    @InjectModel(SyWorkbookFile.name) private readonly syDocModel: Model<SyWorkbookDocumentDocument>,
  ...
  onModuleInit(): void {
    ...
    const bucket = process.env["MINIO_SY_DOCUMENTS_BUCKET"] ?? "sy-workbook-documents";
  ...
  private async loadAuthorize(workbookId: string, acting: ActingUser, requireWrite: boolean): Promise<{ myRoles: WorkbookRoleName[] }> {
    const wb = await this.syWorkbookModel.findOne({ workbookId }).exec();
    if (!wb) throw new NotFoundException("SY workbook not found");
  ...
  async list(workbookId: string, acting: ActingUser): Promise<SyDocumentEntry[]> {
    await this.loadAuthorize(workbookId, acting, false);
    const docs = await this.syDocModel.find({ workbookId }).sort({ createdAt: 1 }).exec();
    return docs.map((d) => ({
  ...
  async upload(workbookId: string, input: UploadInput, acting: ActingUser): Promise<SyDocumentEntry> {
    ...
    const created = await this.syDocModel.create({
      documentId,
      workbookId,
