...
- [x] Receive manual visual and interaction acceptance for the fault-tree editor.
- [x] Reproduce and resolve every remaining Bayesian-network editor issue and add focused regression coverage.
- [x] Receive manual visual and interaction acceptance for the Bayesian-network editor.
- [x] Reproduce and resolve every remaining event-tree editor issue and add focused regression coverage.
- [x] Receive manual visual and interaction acceptance for the event-tree editor.
...
- [x] Obtain final acceptance for all three editors before checking or implementing any remaining Phase 8 workbook-connection TODO.
...
Completion rule: every workbook-connection item must also extend every applicable shipped example, including the temporary dissertation-source example whenever it supports that connection. The connection is not complete until coordinated example generation replaces seed placeholders with real workbook references and a regression executes the connected example path.
...
- [x] Connect IE FT views to their controlled workbook data or typed SY references.
- [x] Link DA workbooks to basic-event parameters and probabilities.
- [x] Link HRA workbooks to human-failure events and HEPs.
...
- [x] Link ESQ workbooks to the exact contributing workbooks, model entities, revisions, and immutable analysis runs.
- [x] Link RC/RI workbooks to end-state and risk results.
- [x] Link hazard PRA workbooks to hazard-conditioned models.
...
| --- | --- | --- | --- |
| 2026-08-25 | Link hazard PRA workbooks to hazard-conditioned models | Added a strict workbook-owned hazard-conditioned model bundle with initiating-event fault trees, a shared basic-event catalogue, event trees and external sequence records, and dependency Bayesian networks, including legacy-workbook defaults. All six hazard PRA workbooks now host the canonical editors in their initiating-event/plant-response, event-sequence, human-reliability, dependency, and quantification steps as appropriate; SY Step 05, ES Step 03, and HR Step 09 now own and edit dependency BNs through the same canonical component. Coordinated SFR and HTGR generation populates every supported hazard and core dependency host, while the dissertation-source example now extends through SY, ES, ESQ, and HR dependency BNs. Connectivity/schema regressions passed for all 12 hazard example variants and the core examples; complete frontend, backend, and shared-interface suites passed 623, 446, and 667 tests; affected typechecks and lints passed; both production builds completed successfully | Passed |
| 2026-08-25 | Link ESQ workbooks to exact contributors and immutable analysis runs | Added strict run-target and contribution contracts that bind every BN, HCL-FT, and HCL-ET execution to its ESQ owner snapshot, exact requested model entities, and every contributing workbook revision. Immutable run records now retain typed references for BN nodes, HCL bindings, fault-tree events and top events, event-tree functional events, DA parameters, and HRA HFE/HEP quantifications. ESQ Step 05 exposes the access-controlled run history in a compact provenance panel with nested exact-reference details. Native PRAXIS regressions executed the connected SFR, HTGR, and dissertation-source examples and verified their exact provenance through the new ESQ endpoint. Complete frontend and backend suites passed 620 and 418 tests; the complete shared-interface suite passed 659 tests; affected typechecks, lints, production builds, formatting, and diff checks passed | Passed |
| 2026-08-25 | Link HRA workbooks to human-failure events and HEPs | Added a strict `HUMAN_FAILURE_EVENT` reference that identifies the HRA workbook, HFE, and exact HEP quantification. SY human-error basic events and HFE integrations now use project HRA pickers, display the selected HEP, and preserve the reference through canonical FT projection and OpenPSA interchange. Native FT, ET, HCL-FT, and HCL-ET runs resolve the current HEP, reject missing or mismatched quantifications, authorize the source workbook, and capture its immutable revision and MEF. HRA persistence now uses revisioned patches and atomic revision changes. Coordinated generation reconciles SFR, HTGR, and dissertation-source HRA examples into SY; the dissertation-source set now includes a two-HFE HRA workbook. A native PRAXIS regression quantitatively combined DA and HRA controlled events, recorded both source revisions, and executed the dissertation-source FT plus all three event trees. Complete frontend, backend, and shared-interface suites passed 618, 415, and 658 tests; all four affected typechecks and lints plus both production builds passed | Passed |
| 2026-08-25 | Link DA workbooks to basic-event parameters and probabilities | Added typed `WORKBOOK_PARAMETER` control references to the SY basic-event catalogue and a project DA-parameter picker that accepts only finite probability, unavailability, and HEP parameters. FT canvases and drawers project the current DA value; native FT, ET, HCL-FT, and HCL-ET runs resolve reachable controlled parameters from DA, authorize every contributing workbook, reject missing/incompatible/out-of-range values, and capture the exact DA revision and MEF in the immutable run context. DA persistence is now revisioned so those snapshots are meaningful. Extended coordinated example generation to reconcile matching DA parameters into SY basic events for every shipped example, added a 25-parameter DA workbook to the dissertation-source IE/SY/DA/ES/ESQ set, and executed its connected FT plus all three HCL event trees with the DA revision preserved. Focused UI, adapter, run, immutable-context, revision, seed, and example-generation suites passed; complete frontend, backend, and shared-interface suites passed 615, 410, and 653 tests; all four affected typechecks and lints, both production builds, formatting, and diff checks passed | Passed |
| 2026-08-25 | Close editor acceptance and reconcile the first Phase 8 IE connection | Recorded the user's final visual and interaction acceptance after the completed FT, BN, ET, HCL, result-presentation, and shipped-example refinements. Repository inspection confirmed that IE Step 02 derives its read-only MLD/HBFT diagrams from the owning IE workbook and that IE frequency fault-tree operations round-trip through the canonical editor adapter into `FrequencyDataSource.faultTree` and the revisioned IE workbook patch path. The two focused IE adapter suites passed 8/8 tests, covering MLD/HBFT source traceability plus every frequency-tree node/gate type, shared DAG inputs, catalogue-only events, probabilities, transfer targets, canonical edits, and strict schema validation | Passed |
