...
- [ ] Find models and workbooks that reference another model.
...
- [ ] Create `apps/solvers/praxis-node` as a NAPI-RS addon.
- [ ] Use Node-API and depend directly on the PRAXIS Rust crate.
- [ ] Use TensorBayes through PRAXIS.
- [ ] Accept versioned JSON requests and return versioned JSON results.
- [ ] Return structured validation and solver errors.
- [ ] Expose only `validate(requestJson)` and `execute(requestJson)` publicly.
- [ ] Load the addon in a Praetor worker so solver work never runs on the main backend event loop.
- [ ] Build and load the addon on Windows.
- [ ] Build and load the addon on Linux.
- [ ] Verify Docker addon packaging.
