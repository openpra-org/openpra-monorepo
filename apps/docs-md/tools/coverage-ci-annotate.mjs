...
const COVERAGE_JSON = join(ROOT, 'apps/docs-md/api/ts/coverage.json');
const THRESHOLDS_JSON = join(ROOT, 'apps/docs-md/tools/coverage-thresholds.json');
...
function main() {
    ...
    const mdLines = [];
    if (tsRows.length > 0) {
        mdLines.push('## TypeScript docs coverage (symbol-level)', '', '| package | doc% | params% | returns% | symbols | documented |', '|---|---:|---:|---:|---:|---:|', ...tsRows.map((r) => `| ${r.package} | ${pct(r.symbolsPercent)}% | ${pct(r.paramsPercent)}% | ${pct(r.returnsPercent)}% | ${r.symbols ?? 0} | ${r.symbolsDocumented ?? 0} |`), '');
    }
    if (cppCats.length > 0) {
        mdLines.push('## C++ docs coverage (heuristic, Doxybook2)', '', '| category | files | contentful | ratio |', '|---|---:|---:|---:|', ...cppCats.map((c) => `| ${c.category} | ${c.files ?? 0} | ${c.contentful ?? 0} | ${pct(c.ratioPercent)}% |`), '');
    }
