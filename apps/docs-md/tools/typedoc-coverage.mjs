...
const CPP_DOCS_DIR = join(ROOT, 'apps/docs-md/api/cpp-doxybook2');
    ...
    for (const pkg of pkgDirs) {
        ...
        function anyDescendantHasComment(node) {
            ...
            if (Array.isArray(node.signatures))
                stacks.push(...node.signatures);
            if (Array.isArray(node.getters))
                stacks.push(...node.getters);
            if (Array.isArray(node.setters))
                stacks.push(...node.setters);
            while (stacks.length > 0) {
                const n = stacks.pop();
                if (hasComment(n))
                    return true;
                if (Array.isArray(n?.children))
                    stacks.push(...n.children);
                if (Array.isArray(n?.signatures))
                    stacks.push(...n.signatures);
                if (Array.isArray(n?.getters))
                    stacks.push(...n.getters);
                if (Array.isArray(n?.setters))
                    stacks.push(...n.setters);
            }
            return false;
        }
        function collectSignatureMetrics(node) {
            ...
            if (Array.isArray(node.children)) {
                for (const ch of node.children)
                    collectSignatureMetrics(ch);
            }
            if (Array.isArray(node.getters)) {
                for (const g of node.getters)
                    collectSignatureMetrics(g);
            }
            if (Array.isArray(node.setters)) {
                for (const s of node.setters)
                    collectSignatureMetrics(s);
            }
        }
...
function main() {
    ...
    try {
        ...
        writeFileSync(jsonPath, JSON.stringify({ generatedAt: new Date().toISOString(), ts, cpp }, null, 2));
        const mdTsLines = [
        ...
        writeFileSync(jsonCppPath, JSON.stringify({ generatedAt: new Date().toISOString(), cpp }, null, 2));
    }
    catch (e) {
