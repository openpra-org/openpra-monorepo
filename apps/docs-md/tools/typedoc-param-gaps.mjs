...
function gatherParamGaps(model, pkg) {
    ...
    function annotateParents(node, parent) {
        ...
        if (Array.isArray(node.children))
            node.children.forEach((c) => annotateParents(c, node));
        if (Array.isArray(node.signatures))
            node.signatures.forEach((s) => annotateParents(s, node));
        if (Array.isArray(node.getters))
            node.getters.forEach((g) => annotateParents(g, node));
        if (Array.isArray(node.setters))
            node.setters.forEach((s) => annotateParents(s, node));
        if (Array.isArray(node.parameters))
            node.parameters.forEach((p) => annotateParents(p, node));
    }
    function walk(node) {
        if (!node || typeof node !== 'object')
            return;
...
function main() {
    ...
    try {
        ...
        writeFileSync(join(TS_DOCS_DIR, 'gaps-params.json'), JSON.stringify({ generatedAt: new Date().toISOString(), perPackage: Object.fromEntries(perPkg), topSymbols, entries: allEntries.slice(0, 200) }, null, 2));
    }
    catch (e) {
