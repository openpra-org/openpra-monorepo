...
function EsIeLinkModal({ workbookId, currentLinkedId, onClose, onConfirm }: {
  workbookId: string;
  currentLinkedId: string | null;
  onClose: () => void;
  onConfirm: (ieWorkbookId: string) => Promise<void>;
}): JSX.Element {
  const [available, setAvailable] = useState<AvailableIeWorkbook[] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    getAvailableIeWorkbooks(workbookId)
      .then((res) => { if (!cancelled) { setAvailable(res); if (currentLinkedId === null && res.length > 0) setPicked(res[0].workbookId); } })
      .catch((err: unknown) => { if (!cancelled) setError((err as { message?: string }).message ?? "Could not load IE workbooks"); });
  ...
  return (
    <div className="eslink-backdrop" onClick={busy ? undefined : onClose}>
      <div className="eslink" role="dialog" aria-modal="true" aria-label="Link an IE workbook" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="eslink__body">
          <div className="eslink__col-h"><span>Choose an IE workbook</span></div>
          {available === null && error === null && <p className="possubtle">Loading IE workbooks…</p>}
          {available !== null && available.length === 0 && <p className="possubtle">No IE workbooks in this project yet. Create and fill an IE workbook first.</p>}
                  ...
                  <button key={w.workbookId} type="button" className={`eslink__pick${active ? " eslink__pick--active" : ""}`} onClick={() => setPicked(w.workbookId)}>
                    ...
                    <span className="eslink__pick-main">
                      ...
                      <span className="eslink__pick-meta">
                        ...
                        <span className="possubtle">{w.initiatorCount} initiators · {w.groupCount} groups</span>
                      </span>
                    </span>
                  </button>
                );
