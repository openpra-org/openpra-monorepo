...
function EsLoadExampleModal({ onCancel, onConfirm }: {
  onCancel: () => void;
    ...
    <div className="posloadex-overlay" onClick={busy ? undefined : onCancel}>
      <div className="posloadex-modal" onClick={(e) => e.stopPropagation()}>
        <header className="posloadex__head">
          ...
          <button type="button" className="posloadex__close" onClick={onCancel} disabled={busy} aria-label="Close"><ESIcon.Close /></button>
        </header>
        <div className="posloadex__body">
          <p>
            This will replace the current workbook contents with the <strong>Generic-1</strong> ES example. Any data
...
function EsUnloadExampleModal({ onCancel, onConfirm }: {
  onCancel: () => void;
  onConfirm: () => Promise<void>;
}): JSX.Element {
  const [busy, setBusy] = useState(false);
