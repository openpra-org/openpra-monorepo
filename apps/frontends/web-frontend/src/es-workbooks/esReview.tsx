...
import { type CommentView, type CcScore } from "./esSelectors";
import { type CapabilityCategory, type EsPersona } from "./esViewData";
...
function InternalReviewScreen({
  ...
  const approver = es.metadata.reviewers.find((r) => r.role === "INTERNAL_APPROVER") ?? null;
  const configSnapshotId = es.configurationControlRecordId ?? "";
  const methodIds = es.newlyDevelopedMethodIds ?? [];

  const reviewerIds = useMemo(() => es.metadata.reviewers.filter((r) => r.role === "INTERNAL_REVIEWER").map((r) => r.id), [es.metadata.reviewers]);
  const approverIds = useMemo(() => es.metadata.reviewers.filter((r) => r.role === "INTERNAL_APPROVER").map((r) => r.id), [es.metadata.reviewers]);
  ...
  return (
    <>
      <div className={`posrevbanner posrevbanner--${banner}`}>
        <div className="posrevbanner__icon"><ESIcon.Lock /></div>
      ...
      <div className="poscard">
        <div className="poscard__head">
          <WorkbookSectionHeading workbook="ES" title={isApprovalStep ? (isPreparer ? "Comments by reviewers & approvers" : "Your comments") : "All review comments"} cueKey="Review comments" />
      ...
      {isPreparer && isReviewStep && es.workflowState !== "INTERNAL_TECHNICAL_REVIEW" && es.workflowState !== "INTERNAL_APPROVAL" && es.workflowState !== "FINAL" && (
        <div className="poscard">
          <div className="poscard__head">
            <WorkbookSectionHeading workbook="ES" title="Submit for Internal Approval" level={3} />
          ...
          <div className="posrow" style={{ gap: 10, alignItems: "center" }}>
            ...
            {submitted ? (
              ...
              <button type="button" className="posnav__btn posnav__btn--primary" disabled={!allResolved} style={!allResolved ? { opacity: 0.5, cursor: "not-allowed" } : undefined} onClick={() => { if (allResolved) onSubmitToApproval(); }}>
                {allResolved ? <ESIcon.Send /> : <ESIcon.Lock />} Submit for Internal Approval
              </button>
            )}
          </div>
        </div>
      ...
      {isApprovalStep && (
        <>
          <div className="poscard">
            <div className="poscard__head"><WorkbookSectionHeading workbook="ES" title="What is being attested" level={3} /></div>
              ...
              <div className="posapprove__attest-grid">
                ...
                <div className="posapprove__attest-row">
                  ...
                  {configSnapshotId.length > 0 ? (
                    <button type="button" className="posapprove__attest-val poscclink" onClick={() => onAction("Configuration Control workbook — coming soon")}><ESIcon.Lock /> {configSnapshotId}</button>
                  ) : (
                ...
                <div className="posapprove__attest-row">
                  ...
                  {methodIds.length > 0 ? (
                    <div className="posrow posrow--wrap" style={{ gap: 6 }}>{methodIds.map((nmId) => <button key={nmId} type="button" className="poschip poschip--method" onClick={() => onAction(`${nmId} — Newly Developed Method workbook coming soon`)}><ESIcon.Bolt /> {nmId}</button>)}</div>
                  ) : (
  ...
  return (
    <>
      <button type="button" className={`posrevdock-fab${open ? " posrevdock-fab--open" : ""}`} onClick={onToggle} aria-label="Reviewer comments">
        <ESIcon.History /><span className="posrevdock-fab__label">Comments</span><span className="posrevdock-fab__count">{openCount}</span>
      </button>
      {open && (
        <aside className="posrevdock" role="dialog" aria-label="Reviewer comment dock">
          <div className="posrevdock__head">
            ...
            <button type="button" className="posdrawer__close" onClick={onClose} aria-label="Close"><ESIcon.Close /></button>
          </div>
                ...
                <div className="posrevdock__item-foot">
                  ...
                  {isReviewer && (
                    <button type="button" className={`posnav__btn posnav__btn--sm${c.resolved ? "" : " posnav__btn--primary"}`} onClick={() => onToggleResolved(c.id)}>
                      {c.resolved ? <><ESIcon.Close /> Reopen</> : <><ESIcon.Check /> Mark resolved</>}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="posrevdock__composer">
            ...
            <div className="posrevdock__composer-foot">
              ...
              <button type="button" className="posnav__btn posnav__btn--sm posnav__btn--primary" disabled={draft.trim() === ""} style={draft.trim() === "" ? { opacity: 0.5 } : undefined} onClick={() => { onPostComment(draft.trim(), draftSeverity); setDraft(""); }}>
                <ESIcon.Send /> Post comment
              </button>
            </div>
            {canRequestRevision && (
              <div style={{ marginTop: 8 }}>
                <button type="button" className="posnav__btn posnav__btn--sm" onClick={onRequestRevision}><ESIcon.Close /> Request revision (return to preparer)</button>
              </div>
            )}
          </div>
        </aside>
