...
function InternalReviewScreen({
  ...
  return (
    <>
                  ...
                  <div key={c.id} className={`poscomment poscomment--${c.severity.toLowerCase()} poscomment--${c.resolved ? "resolved" : "open"}`}>
                    ...
                    <div className="poscomment__main">
                      <div className="poscomment__head">
                        ...
                        {c.severity === "MAJOR" && <Badge kind="block">Major</Badge>}
                        {c.severity === "MINOR" && <Badge kind="warn">Minor</Badge>}
                        {c.severity === "OBSERVATION" && <Badge>Observation</Badge>}
                        {!c.resolved && <Badge kind="progress">Open</Badge>}
                        {c.resolved && <Badge kind="ok">Resolved</Badge>}
                      </div>
      ...
      {isPreparer && isReviewStep && es.workflowState !== "INTERNAL_TECHNICAL_REVIEW" && es.workflowState !== "INTERNAL_APPROVAL" && es.workflowState !== "FINAL" && (
        <div className="poscard">
          ...
          <div className="posrow" style={{ gap: 10, alignItems: "center" }}>
            ...
            {submitted ? (
              <Badge kind="ok">Submitted · awaiting approver</Badge>
            ) : (
      ...
      {isApprover && !isApprovalStep && (
        <div className="poscard">
          <div className="poscard__head"><h3 className="poscard__title">Approver view</h3></div>
          <p className="possubtle" style={{ margin: 0 }}>Navigate to the Approval step (Step 11) to sign the workbook once the review is complete.</p>
        </div>
      )}
    </>
  );
  ...
  return (
    <>
      <button type="button" className={`posrevdock-fab${open ? " posrevdock-fab--open" : ""}`} onClick={onToggle} aria-label="Reviewer comments">
        <ESIcon.Mic /><span className="posrevdock-fab__label">Comments</span><span className="posrevdock-fab__count">{openCount}</span>
      </button>
      {open && (
        <aside className="posrevdock" role="dialog" aria-label="Reviewer comment dock">
          <div className="posrevdock__head">
            <div>
              ...
              <div className="posrevdock__title">All comments ({comments.length})</div>
            </div>
