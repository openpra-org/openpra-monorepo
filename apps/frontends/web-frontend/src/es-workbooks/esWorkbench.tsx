...
function headersFor(stepId: string): StepHeader {
  switch (stepId) {
    case "scope":     return { eyebrow: "Step 01", title: "Scope, sources & safety functions", sub: "Upstream inputs, radioactive sources, barriers, and key safety functions (ES-A1 – ES-A3)." };
    case "sequences": return { eyebrow: "Step 02", title: "Event sequences", sub: "Delineate sequences for each initiating event across every applicable operating state (ES-A5 – ES-A7)." };
    case "deps":      return { eyebrow: "Step 03", title: "Dependencies", sub: "Identify functional, human, phenomenological, operational, and physical-interface dependencies (HLR-ES-B)." };
    case "timing":    return { eyebrow: "Step 04", title: "Timing & phenomena", sub: "Order functional events by progression timing; record harsh-environment phenomenological impacts (ES-A6, ES-B3)." };
    case "endstates": return { eyebrow: "Step 05", title: "End states & release categories", sub: "Define safe stable states and release categories; map physical release characteristics (ES-A8, ES-C1 – ES-C3)." };
    case "families":  return { eyebrow: "Step 06", title: "Sequence families", sub: "Group sequences by shared end state, release category, and plant response for ESQ hand-off (ES-C8)." };
    case "screening": return { eyebrow: "Step 07", title: "Screening", sub: "Retain all sequences by default; only SCR-3 may screen, and never if a barrier is challenged (ES-A7)." };
    case "quant":     return { eyebrow: "Step 08", title: "Quantification", sub: "Mean frequency per family — the values handed off to Event Sequence Quantification and Mechanistic Source Term." };
    case "draft":     return { eyebrow: "Step 09 · Draft", title: "Produce the draft", sub: "Generate the ES report from everything entered. Submitting advances the workbook to internal technical review." };
    case "review":    return { eyebrow: "Step 10 · Review", title: "Internal technical review", sub: "Reviewers leave comments and the preparer responds. The workbook advances to Approval once every comment is resolved." };
...
function WorkspaceHeader({
  persona, setPersona, workflowState, showPersonaPicker, availablePersonas,
  onOpenRoles, onLoadExample, onUnloadExample, headerMeta, onToggleRail, onToggleDock,
}: {
  persona: EsPersona;
  ...
  return (
    <header className={`poshd${isReviewer ? " poshd--external" : ""}${isApprover ? " poshd--approver" : ""}`}>
      ...
      <div className="poshd__crumb">
        ...
        {personaPill !== null ? (
          <span className={`poshd__wfstate ${personaPill.cls}`}><ESIcon.Lock />{personaPill.text}</span>
        ) : (
          <span className="poshd__wfstate poshd__wfstate--draft"><span className="poshd__wfstate-dot" />{workflowState}</span>
        )}
      </div>
      ...
      <div className="poshd__actions">
        ...
        {onOpenRoles !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onOpenRoles}><ESIcon.Settings /> Roles</button>
        )}
        {onLoadExample !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onLoadExample}><ESIcon.Sparkle /> Load example</button>
        )}
        {onUnloadExample !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onUnloadExample}><ESIcon.Close /> Unload example</button>
        )}
        ...
        <div className="posdock__gauge">
          <div className="posdock__gauge-circle">
            <svg viewBox="0 0 36 36">
              ...
              <circle cx="18" cy="18" r="15.9" fill="none" strokeWidth="3.2" className="posdock__gauge-fill"
                strokeDasharray={`${dash} ${dashTotal}`} strokeLinecap="round" transform="rotate(-90, 18, 18)" />
...
function EsWorkbench({
  data, persona, setPersona, showPersonaPicker, availablePersonas = DEFAULT_PERSONAS,
  onOpenRoles, onLoadExample, onUnloadExample, onStageChange, actions, headerMeta,
  renderApprovalTable, renderSignCard, renderRoster,
}: {
  data: EsWorkbookData;
  persona: EsPersona;
  setPersona: (p: EsPersona) => void;
  ...
  function handlePostComment(text: string, severity: "MAJOR" | "MINOR" | "OBSERVATION"): void {
    ...
    actions.postComment(text, severity, STEP_SR_HINT[stepId] ?? stepId).then(() => flash("Comment posted")).catch((err: unknown) => flash((err as { message?: string }).message ?? "Could not post comment"));
  }
  ...
  function renderScreen(): JSX.Element | null {
    switch (stepId) {
      case "scope":     return <ScopeScreen {...screenProps} stage={stage} setStage={setStage} />;
      ...
      case "approval":  return (
        <InternalReviewScreen
          ...
          persona={persona as "preparer" | "reviewer" | "approver"}
          cc={cc}
          scores={scores}
          comments={comments}
          submitted={submitted}
  ...
  return (
    <div className={`posw${isReviewer ? " posw--external posw--reviewer" : ""}${isApprover ? " posw--approver" : ""}`} data-screen-label={`ES — ${step?.label ?? ""}`}>
      ...
      <WorkspaceHeader
        persona={persona} setPersona={setPersona} workflowState={workflowState}
        showPersonaPicker={showPersonaPicker} availablePersonas={availablePersonas}
        onOpenRoles={onOpenRoles} onLoadExample={onLoadExample} onUnloadExample={onUnloadExample}
        headerMeta={headerMeta}
        onToggleRail={() => setRailMobileOpen((v) => !v)}
        onToggleDock={() => { setDockOpen(true); setDockMobileOpen((v) => !v); }}
      />
        ...
        <main className="posmain" aria-label="Step content">
          <div className="posmain__head">
            ...
            <div className="posmain__actions">
              {!dockOpen && (
                <button type="button" className="posnav__btn posnav__btn--sm" onClick={() => { setDockOpen(true); setDockMobileOpen(true); }}>
                  <ESIcon.Eye /> Show conformance
                </button>
              )}
              {isPreparer && stepId !== "draft" && stepId !== "review" && stepId !== "approval" && (
                <button type="button" className="posnav__btn posnav__btn--sm" onClick={() => setStepId("draft")}>
                  Preview draft <ESIcon.Eye />
                </button>
              )}
            </div>
          </div>
          ...
          <div className="posnav">
            <button type="button" className={`posnav__btn posnav__btn--sm${prev !== undefined ? "" : " posnav__btn--ghost"}`} disabled={prev === undefined} onClick={() => { if (prev !== undefined) setStepId(prev.id); }}>
              <ESIcon.ArrowL /> {prev !== undefined ? prev.label : "Start"}
            </button>
            {next !== undefined ? (
              <button type="button" className="posnav__btn posnav__btn--sm posnav__btn--primary" onClick={() => setStepId(next.id)}>
                Next: {next.label} <ESIcon.ArrowR />
              </button>
            ) : isPreparer && approved ? (
              <button type="button" className="posnav__btn posnav__btn--sm posnav__btn--primary" onClick={() => flash("Released to external workflows")}>
                <ESIcon.Send /> Released to external workflows
              </button>
            ) : <span />}
          </div>
        </main>
...
export { EsWorkbench, type EsWorkbenchActions, type HeaderMeta };
