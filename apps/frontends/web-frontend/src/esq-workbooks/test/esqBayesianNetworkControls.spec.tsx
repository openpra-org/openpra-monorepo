...
function Workbook({ editable = true, initial = empty, changed = jest.fn() }: {
  editable?: boolean; initial?: EventSequenceQuantification; changed?: jest.Mock;
}) {
...
it("explains linked-network restrictions beside visible controls and links to the owner", async () => {
  ...
  jest.mocked(listWorkbooks).mockImplementation(async (_project, element) => ({ workbooks: [{ id: element === "SY" ? "source-sy" : "source-es", name: "Source systems" }] }) as Awaited<ReturnType<typeof listWorkbooks>>);
  ...
  expect(screen.getByRole("link", { name: "Open Systems workbook" })).toHaveAttribute("href", "/sy-workbooks/source-sy");
