...
import { Badge } from "./hrShared";
import { type CommentView, type CcScore } from "./hrSelectors";
import { type CapabilityCategory, type HrPersona } from "./hrViewData";
...
function InternalReviewScreen({
  ...
  const approver = hr.metadata.reviewers.find((r) => r.role === "INTERNAL_APPROVER") ?? null;
  const configSnapshotId = hr.configurationControlRecordId ?? "";
  const methodIds = hr.newlyDevelopedMethodIds ?? [];

  const reviewerIds = useMemo(() => hr.metadata.reviewers.filter((r) => r.role === "INTERNAL_REVIEWER").map((r) => r.id), [hr.metadata.reviewers]);
  const approverIds = useMemo(() => hr.metadata.reviewers.filter((r) => r.role === "INTERNAL_APPROVER").map((r) => r.id), [hr.metadata.reviewers]);
  ...
  return (
    <>
      <div className={`posrevbanner posrevbanner--${banner}`}>
        <div className="posrevbanner__icon"><HRIcon.Lock /></div>
      ...
      <div className="poscard">
        <div className="poscard__head">
          <WorkbookSectionHeading workbook="HR" title={isApprovalStep ? (isPreparer ? "Comments by reviewers & approvers" : "Your comments") : "All review comments"} cueKey="Review comments" />
      ...
      {isPreparer && isReviewStep && hr.workflowState !== "INTERNAL_TECHNICAL_REVIEW" && hr.workflowState !== "INTERNAL_APPROVAL" && hr.workflowState !== "FINAL" && (
        <div className="poscard">
          <div className="poscard__head">
            <WorkbookSectionHeading workbook="HR" title="Submit for Internal Approval" level={3} />
          ...
          <div className="posrow" style={{ gap: 10, alignItems: "center" }}>
            ...
            {submitted ? (
              ...
              <button type="button" className="posnav__btn posnav__btn--primary" disabled={!allResolved} style={!allResolved ? { opacity: 0.5, cursor: "not-allowed" } : undefined} onClick={() => { if (allResolved) onSubmitToApproval(); }}>
                {allResolved ? <HRIcon.Send /> : <HRIcon.Lock />} Submit for Internal Approval
              </button>
            )}
          </div>
        </div>
      ...
      {isApprovalStep && (
        <>
          <div className="poscard">
            <div className="poscard__head"><WorkbookSectionHeading workbook="HR" title="What is being attested" level={3} /></div>
              ...
              <div className="posapprove__attest-grid">
                ...
                <div className="posapprove__attest-row">
                  ...
                  {configSnapshotId.length > 0 ? (
                    <button type="button" className="posapprove__attest-val poscclink" onClick={() => onAction("Configuration Control workbook — coming soon")}><HRIcon.Lock /> {configSnapshotId}</button>
                  ) : (
                ...
                <div className="posapprove__attest-row">
                  ...
                  {methodIds.length > 0 ? (
                    <div className="posrow posrow--wrap" style={{ gap: 6 }}>{methodIds.map((nmId) => <button key={nmId} type="button" className="poschip poschip--method" onClick={() => onAction(`${nmId} — Newly Developed Method workbook coming soon`)}><HRIcon.Bolt /> {nmId}</button>)}</div>
                  ) : (
  ...
  return (
    <>
      <button type="button" className={`posrevdock-fab${open ? " posrevdock-fab--open" : ""}`} onClick={onToggle} aria-label="Reviewer comments">
        <HRIcon.History /><span className="posrevdock-fab__label">Comments</span><span className="posrevdock-fab__count">{openCount}</span>
      </button>
      {open && (
        <aside className="posrevdock" role="dialog" aria-label="Reviewer comment dock">
          <div className="posrevdock__head">
            ...
            <button type="button" className="posdrawer__close" onClick={onClose} aria-label="Close"><HRIcon.Close /></button>
          </div>
                ...
                <div className="posrevdock__item-foot">
                  ...
                  {isReviewer && (
                    <button type="button" className={`posnav__btn posnav__btn--sm${c.resolved ? "" : " posnav__btn--primary"}`} onClick={() => onToggleResolved(c.id)}>
                      {c.resolved ? <><HRIcon.Close /> Reopen</> : <><HRIcon.Check /> Mark resolved</>}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="posrevdock__composer">
            ...
            <div className="posrevdock__composer-foot">
              ...
              <button type="button" className="posnav__btn posnav__btn--sm posnav__btn--primary" disabled={draft.trim() === ""} style={draft.trim() === "" ? { opacity: 0.5 } : undefined} onClick={() => { onPostComment(draft.trim(), draftSeverity); setDraft(""); }}>
                <HRIcon.Send /> Post comment
              </button>
            </div>
            {canRequestRevision && (
              <div style={{ marginTop: 8 }}>
                <button type="button" className="posnav__btn posnav__btn--sm" onClick={onRequestRevision}><HRIcon.Close /> Request revision (return to preparer)</button>
              </div>
            )}
          </div>
        </aside>
