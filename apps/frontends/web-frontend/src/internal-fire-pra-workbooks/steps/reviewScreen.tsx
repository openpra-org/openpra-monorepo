...
const REVIEW_SECTIONS: Record<string, string> = {
  FPP: "Plant partitioning",
  FES: "Equipment selection",
  FCS: "Cable selection and routing",
  FQLS: "Qualitative screening",
  FPRM: "Plant response",
  FSS: "Fire scenarios",
  FIGN: "Ignition frequency",
  FCF: "Circuit failure",
  FHR: "Human reliability",
  FESQ: "Quantification and uncertainty",
};
...
interface ReviewCommentsProps {
  ...
  persona: InternalFirePraPersona;
  actions?: InternalFireWorkflowActions;
      ...
      const nextComments = current.internalReviewComments.comments.map((entry) => {
        ...
        if (nextResolved) {
          return {
            ...entry,
            resolved: true,
            resolution: entry.resolution ?? "The responsible analyst incorporated the review disposition and the comment author verified the revised analysis record.",
            resolvedAt: new Date().toISOString(),
            resolvedBy: persona === "approver" ? "Internal Fire approver" : "Internal Fire technical reviewer",
          };
        }
...
export function ReviewScreen({ persona, actions, renderRoster }: { persona: InternalFirePraPersona; actions?: InternalFireWorkflowActions; renderRoster?: () => ReactNode }): JSX.Element {
  const { mef, mutate } = useInternalFirePraWorkbook();
  ...
  function submit(): void {
    ...
    if (operation !== undefined) operation.finally(() => setBusy(false));
      ...
      mutate((current) => ({ ...current, workflowState: "INTERNAL_APPROVAL", workflowHistory: [...current.workflowHistory, { state: "INTERNAL_APPROVAL", enteredAt: new Date().toISOString(), actor: current.owner ?? "Internal Fire PRA Team" }] }));
      setBusy(false);
    }
  }
