...
export function InternalFloodPraDemoPage(): JSX.Element {
  ...
  return <InternalFloodPraWorkbookProvider mef={mef} editable={persona === "preparer"} mutate={mutate}><InternalFloodPraWorkbench persona={persona} setPersona={setPersona} headerMeta={{ projectName: mef.metadata.plantIdentity?.name ?? "Reference plant", workbookName: mef.name, workbookVersion: mef.version }} renderSignCard={() => <WorkbookDemoSignCard persona={persona} myOpenComments={0} submitted preparerPhase={false} onReviewerApproverSigned={() => undefined} onPreparerSigned={() => undefined} />} exampleOptions={examples} selectedExample={selected} onSelectExample={selectExample} /></InternalFloodPraWorkbookProvider>;
}
