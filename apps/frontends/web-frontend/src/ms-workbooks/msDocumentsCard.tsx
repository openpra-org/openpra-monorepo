...
function MsDocumentsCard({ workbookId, canEdit }: { workbookId?: string; canEdit: boolean }): JSX.Element {
  const { ms } = useMsWorkbook();
  const exampleDocs = ms.exampleDocuments ?? [];
  const [docs, setDocs] = useState<MsDocumentEntry[] | null>(workbookId === undefined ? [] : null);
  ...
  async function refresh(): Promise<void> {
    if (workbookId === undefined) return;
    setDocs(await listMsDocuments(workbookId));
  }
  ...
  function onUpload(e: ChangeEvent<HTMLInputElement>): void {
    ...
    setBusy(true);
    setError(null);
    uploadMsDocument(workbookId, file)
      .then(refresh)
  ...
  function onDelete(documentId: string): void {
    if (workbookId === undefined) return;
    deleteMsDocument(workbookId, documentId)
      .then(refresh)
  ...
  function onDownload(documentId: string): void {
    if (workbookId === undefined) return;
    getMsDocumentDownload(workbookId, documentId)
  ...
  return (
    <div className="poscard">
      ...
      <p className="poscard__sub">The source and barrier inventory, the transient timing, the sequence families and the bounding behavior that support this analysis.</p>
            ...
            {uploads.map((d) => (
              <tr key={d.documentId}>
                ...
                <td style={{ textAlign: "right" }}>
                  {canEdit && (
                    <button type="button" className="posnav__btn posnav__btn--sm" title="Remove" onClick={() => onDelete(d.documentId)}><MSIcon.Close /></button>
                  )}
                </td>
              </tr>
            ))}
