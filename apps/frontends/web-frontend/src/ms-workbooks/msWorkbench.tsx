...
import { ccScore, commentsView, filterConformance, groupBySection, stepsFromMef, type CommentView } from "./msSelectors";
import { ScopeScreen, CategoriesScreen, SourcesScreen, TransportScreen, type MsDrawerContext } from "./msScreens";
import { SourceTermScreen, UncertScreen, DraftScreen, DrawerContent, PlaceholderScreen } from "./msScreens2";
import { InternalReviewScreen, ReviewerCommentDock } from "./msReview";
...
function headersFor(stepId: string): StepHeader {
  switch (stepId) {
    ...
    case "sourceterm": return { eyebrow: "Step 05", title: "Source Terms", sub: "The mechanistic calculation and the source-term table (MS-C)." };
    case "uncert": return { eyebrow: "Step 06", title: "Uncertainty & Pre-op", sub: "The fork, the phenomena dependencies and the pre-operational assumptions (MS-D)." };
    case "draft": return { eyebrow: "Step 07 · Draft", title: "Produce the draft", sub: "Build the MS report, then send it to review (MS-E)." };
...
function WorkspaceHeader({
  ...
  persona: MsPersona;
  setPersona: (p: MsPersona) => void;
  workflowState: string;
  showPersonaPicker: boolean;
  availablePersonas: MsPersona[];
  onOpenRoles?: () => void;
  onLoadExample?: () => void;
  ...
  return (
    <header className={`poshd${isReviewer ? " poshd--external" : ""}${isApprover ? " poshd--approver" : ""}`}>
      {onToggleRail !== undefined && (
        <button type="button" className="posw__mobile-toggle" onClick={onToggleRail} aria-label="Open steps"><MSIcon.Layers /> Steps</button>
      )}
      <div className="poshd__crumb">
        <button type="button" onClick={() => navigate(-1)}><MSIcon.ArrowL /></button>
        <button type="button" onClick={() => navigate(-1)}>{headerMeta.projectName}</button>
        <MSIcon.Chevron />
        <span>Mechanistic Source Term Analysis</span>
        <MSIcon.Chevron />
        <span className="poshd__crumb-current">{headerMeta.workbookName}</span>
        {personaPill !== null ? (
          <span className={`poshd__wfstate ${personaPill.cls}`} title={MS_PERSONAS[persona].blurb}>
            <MSIcon.Lock />{personaPill.text}
          </span>
        ) : (
      ...
      <div className="poshd__actions">
        {showPersonaPicker && availablePersonas.length > 1 && (
          <label className="poshd__perspective" title="Switch perspective">
            ...
            <select className="poshd__perspective-select" value={persona} onChange={(e) => setPersona(e.target.value as MsPersona)}>
        ...
        {onOpenRoles !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onOpenRoles} title="Manage roles"><MSIcon.Settings /> Roles</button>
        )}
        {onLoadExample !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onLoadExample} title="Replace contents with the Generic-1 example workbook"><MSIcon.Sparkle /> Load example</button>
        )}
        {onUnloadExample !== undefined && (
          <button type="button" className="posnav__btn posnav__btn--sm" onClick={onUnloadExample} title="Restore the contents that existed before the example was loaded"><MSIcon.Close /> Unload example</button>
        )}
        <span className="poshd__save-pill"><span className="poshd__save-pill-dot" />Autosaved · v{headerMeta.workbookVersion}</span>
        <button type="button" className="posnav__btn" aria-label="History"><MSIcon.History /></button>
        {onToggleDock !== undefined && (
          <button type="button" className="posw__mobile-toggle" onClick={onToggleDock} aria-label="Open conformance"><MSIcon.Eye /> Conformance</button>
        )}
      </div>
    </header>
  );
...
function StepRail({ stepId, setStepId, persona, visibleSteps, mobileOpen }: {
  ...
  return (
    <aside className={`posw__rail${mobileOpen ? " posw__rail--mobile-open" : ""}`} aria-label="MS analysis steps">
          ...
          return (
            <li key={s.id}>
              <button type="button" className={`posrail__step${active ? " posrail__step--active" : ""}${complete ? " posrail__step--complete" : ""}${idle ? " posrail__step--idle" : ""}`} onClick={() => setStepId(s.id)}>
                <span className="posrail__step-num">{complete ? <MSIcon.Check /> : s.num}</span>
      ...
      <div className="posrail__footer">
        <button type="button" className="posrail__footer-btn"><MSIcon.Layers /> Show all inputs</button>
        <button type="button" className="posrail__footer-btn"><MSIcon.Settings /> Workbook settings</button>
      </div>
    </aside>
  );
}
...
function ConformanceDock({ ccId, stage, onGoToScope, onClose, mobileOpen }: {
  ...
  const items = useMemo(() => filterConformance(ms, ccId, stage), [ms, ccId, stage]);
  const sections = useMemo(() => groupBySection(items), [items]);
  const scores = ccScore(ms, ccId, stage);
    ...
    <aside className={`posw__dock${mobileOpen ? " posw__dock--mobile-open" : ""}`} aria-label="Conformance checklist">
      <div className="posdock__head">
        <div className="posdock__title-row">
          ...
          <button type="button" className="posdock__close" onClick={onClose} aria-label="Hide checklist"><MSIcon.Close /></button>
        </div>
...
function MsDrawer({ context, onClose }: { context: MsDrawerContext; onClose: () => void }): JSX.Element {
  useEffect(() => {
...
const DEFAULT_PERSONAS: MsPersona[] = ["preparer", "reviewer", "approver"];

function MsWorkbench({
  ...
  const visibleSteps = useMemo(() => stepsFromMef(data.ms, persona), [data.ms, persona]);
  const mefCcId = data.ms.capabilityCategory === "CC-I" ? "cc-i" : "cc-ii";
  const mefStage: Stage = data.ms.plantStage === "OPERATIONAL" ? "operational" : "pre_operational";
  ...
  const [drawer, setDrawer] = useState<MsDrawerContext | null>(null);
  ...
  const [comments, setComments] = useState<CommentView[]>(() => commentsView(data.ms));
  ...
  useEffect(() => { setComments(commentsView(data.ms)); }, [data.ms]);
  ...
  const cc = CAPABILITY_CATEGORIES.find((c) => c.id === ccId) ?? CAPABILITY_CATEGORIES[0];
  const scores = ccScore(data.ms, ccId, stage);
  ...
  return (
    <div className={`posw${isReviewer ? " posw--external posw--reviewer" : ""}${isApprover ? " posw--approver" : ""}`} data-screen-label={`MS — ${step.label}`}>
      {isReviewer && <div className="poshd__extbar" />}
      {isApprover && <div className="poshd__apprbar" />}
      <WorkspaceHeader persona={persona} setPersona={setPersona} workflowState={data.ms.workflowState} showPersonaPicker={showPersonaPicker} availablePersonas={availablePersonas} onOpenRoles={onOpenRoles} onLoadExample={onLoadExample} onUnloadExample={onUnloadExample} headerMeta={headerMeta} onToggleRail={() => setRailMobileOpen((v) => !v)} onToggleDock={() => { setDockOpen(true); setDockMobileOpen((v) => !v); }} />
        ...
        <main className="posmain" aria-label="Step content">
          <div className="posmain__head">
            ...
            <div className="posmain__actions">
              {!dockOpen && (
                <button type="button" className="posnav__btn posnav__btn--sm" onClick={() => { setDockOpen(true); setDockMobileOpen(true); }}><MSIcon.Eye /> Show conformance</button>
              )}
            </div>
          </div>
      ...
      {drawer !== null && <MsDrawer context={drawer} onClose={() => setDrawer(null)} />}
...
export { MsWorkbench, type HeaderMeta, type MsWorkbenchActions };
