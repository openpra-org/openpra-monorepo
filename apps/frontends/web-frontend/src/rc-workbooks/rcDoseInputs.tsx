...
export function RcDosePanel(): JSX.Element {
  ...
  return <div className="poscard rc-dose-input-card"><div className="poscard__head"><WorkbookSectionHeading workbook="RC" title="OpenRC dose inputs" level={3} /></div>
    <section className="rc-dose" aria-label="OpenRC dose inputs">
      ...
      }}>{(["exposure", "coefficients", "response"] as const).map(t => <button key={t} type="button" id={`${id}-${t}`} role="tab" aria-selected={tab === t} aria-controls={`${id}-panel`} tabIndex={tab === t ? 0 : -1} onClick={() => { setTab(t); setRawFile(undefined); }}>{t === "exposure" ? "Exposure settings" : t === "coefficients" ? "Dose coefficients" : "Response calculation"}</button>)}</div>
      <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-${tab}`} tabIndex={0}>
        {tab === "exposure" ? <><div className="ds-file"><div className="ds-file-info"><span className="ds-small">Exposure settings file</span><strong>{saved?.exposure?.file.filename ?? "No file selected"}</strong><span className="ds-small">.inp · .txt</span></div><div className="ds-file-actions">{uploadButton("exposure", Boolean(saved?.exposure))}{saved?.exposure && viewButton(saved.exposure.file)}</div></div>{fileView()}
          ...
          <div className="ds-section-title"><h3>Step 02 response</h3></div><table aria-label="Response calculation inputs"><tbody><tr><th>Response model</th><td>{responseModel ? `Revision ${responseModel.revision} · ${responseModel.cohorts.length} cohorts` : "Not saved"}</td></tr><tr><th>Site grid</th><td>{site?.geometry?.kind === "cells" ? `${site.geometry.radiiKm.length} bands · ${site.geometry.sectors} sectors` : "Radial grid needed"}</td></tr><tr><th>Integration time</th><td>{saved?.settings ? `${show(saved.settings.integrationSeconds)} s` : "Not saved"}</td></tr></tbody></table>
          <p className="ds-note">The file supplies plume arrival, precipitation, projected dose, target organ, reference breathing rate and unprotected Sv/s by cell and time. Step 02 supplies population, actions and protection factors.</p>
