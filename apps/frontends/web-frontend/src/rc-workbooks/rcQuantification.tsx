...
import { caseChecks, caseDatasets, caseDuration, caseFiles, caseStepNames, caseSteps, caseSummaryRows, caseVersions, currentRcCase } from "interfaces-shared-types/rc-workbooks/case-records";
...
const steps = { source: "handoff", site: "protective", weather: "weather", transport: "dispersion", dose: "dose" };
...
export function RcQuantificationPanel({ onOpenStep }: { onOpenStep?: (id: string) => void }): JSX.Element {
  ...
  const files = inspected ? review.data?.files ?? [] : caseFiles(current), preparedChecks = inspected ? review.data?.checks ?? [] : checks;
  ...
  return <div className="poscard rc-case-input-card"><div className="poscard__head"><WorkbookSectionHeading workbook="RC" title="Case inputs and results" level={3} /></div>
    <section className="rc-quantification" aria-label="Case inputs and results">
      ...
      <div role="tabpanel" id={`${id}-panel`} aria-labelledby={`${id}-${tab}`} tabIndex={0}>
        {tab === "checks" && <><ol className="q-input-list">{caseSteps.map((key, i) => <li key={key}><div><button type="button" className="posnav__btn posnav__btn--sm" disabled={!onOpenStep} onClick={() => openStep(key)}>{String(i + 1).padStart(2, "0")}. {caseStepNames[key]}</button></div><div><strong>{key === "dose" && caseDuration(current) !== undefined ? `${show(caseDuration(current)!)} s integration time` : summaries[i][0]}</strong><small>{summaries[i][1]}</small></div><span className="q-status">{checks[i].items.length ? "To review" : "Checked"}</span></li>)}</ol>
          <div className="q-footer"><span className="q-note">{checks.slice(0, 5).filter(c => !c.items.length).length} of 5 input sections checked</span><button type="button" className="posnav__btn posnav__btn--sm posnav__btn--primary" onClick={() => setChecked(true)}>Check case</button></div>
        ...
        {tab === "prepared" && <>
          ...
          <label className="q-input-group">Input group<select className="posfield__select" aria-label="Input group" value={dataset} onChange={e => { setDataset(e.target.value as RcCaseDataset); setOffset(0); }}>{Object.entries(caseDatasets).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select></label>
          <RemoteStatus remote={table} />{table.data && <>{table.data.total ? <><div className="q-table-wrap"><table aria-label="Prepared input records"><thead><tr>{table.data.columns.map(c => <th key={c}>{c}</th>)}</tr></thead><tbody>{table.data.rows.map((r, i) => <tr key={offset + i}>{r.map((v, j) => <td key={j} title={typeof v === "number" ? String(v) : undefined}>{show(v, dataset === "receptors")}</td>)}</tr>)}</tbody></table></div><Pager offset={table.data.offset} size={5} total={table.data.total} onChange={setOffset} /></> : <p className="q-empty">Complete this step to prepare its records.</p>}</>}
          <RemoteStatus remote={review} />
          <details className="q-check"><summary>Original files · {files.length}</summary><ul className="q-file-list">{files.map(f => <li key={f.file.documentId}><span>{f.file.filename}<small>{f.purpose}</small></span><button className="posnav__btn posnav__btn--sm" type="button" aria-label={`View ${f.file.filename}`} aria-expanded={fileId === f.file.documentId} disabled={!actions} onClick={() => showFile(f.file.documentId)}>{fileId === f.file.documentId ? "Hide" : "View"}</button></li>)}</ul>{!files.length && <p className="q-note">No original files saved for this case.</p>}</details>
          {fileId && <div className="q-file-view"><h3>{fileId === "structured" ? "Workbook case data" : files.find(f => f.file.documentId === fileId)?.file.filename}</h3><RemoteStatus remote={text} />{text.data && <TextPage page={text.data} onChange={setTextOffset} label="Case file content" />}</div>}
