...
export function RcTransportPanel(): JSX.Element {
  ...
  const fileBox = (kind: Tab) => {
    ...
    return <><div className="at-file"><div className="at-file-info"><span className="at-small">{labels[kind]}</span><strong>{kind === "decay" && files.length > 1 ? `${files.length} files` : file?.filename ?? (kind === "deposition" && sourceLoading ? "Reading Step 01 file…" : "No file selected")}</strong><span className="at-small">{extensions[kind]}</span></div>
      <div className="at-file-actions">{editable && <button type="button" className="posnav__btn posnav__btn--sm" disabled={disabled || kind === "deposition" && (dirty || !source)} title={kind === "deposition" && dirty ? "Save or discard edits before importing" : undefined} onClick={() => { importKind.current = kind; input.current?.click(); }}>{kind === "decay" || !file ? <RCIcon.Plus /> : <RCIcon.Refresh />} {kind === "decay" ? "Add files" : file ? "Replace file" : "Import file"}</button>}
    ...
    <section className="rc-transport" aria-label="OpenRC transport inputs">
      ...
      <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-${tab}`} tabIndex={0}>
        {tab === "deposition" ? <>{fileBox("deposition")}{custom && editable && <button type="button" className="posnav__btn posnav__btn--sm" disabled={disabled || dirty} onClick={() => unlink("deposition")}>Use Step 01 file</button>}
          ...
          {!source ? <p className="at-empty">Import or save this category’s source inventory in Step 01.</p> : <><h3>OpenRC group deposition</h3>
            ...
            </> : !sourceLoading && <p className="at-note">{linked?.issue ?? "Import a MelMACCS file to inspect its particle-size data."}</p>}
          ...
          <div className="at-reference">{fileBox("dispersion")}{inputs?.dispersionReference ? <>
            ...
            <p className="at-note">σ<sub>y</sub> = sideways spread · σ<sub>z</sub> = vertical spread</p><dl className="at-readonly"><div><dt>Imported sideways scale</dt><dd>{show(inputs.dispersionReference.data.sidewaysScale)}</dd></div><div><dt>Imported vertical scale</dt><dd>{show(inputs.dispersionReference.data.verticalScale)}</dd></div></dl>
          </> : <p className="at-empty">Import a MACCS input excerpt to review its dispersion parameters.</p>}</div>
          ...
          {!source && <p className="at-note">Add a Step 01 inventory to check its decay-data coverage.</p>}
          ...
          </>}{!parents.length && <p className="at-empty">Add ENSDF files to inspect parent and daughter records.</p>}
        </>}
      </div>
