...
import { type SeismicPraExampleOption } from "./seismicPraWorkbookApi";
...
function SeismicPraDemoPage(): JSX.Element {
  ...
  const [selected, setSelected] = useState("htgr");
  useEffect(() => { let cancelled = false; Promise.all([fetchJson<BundleResponse>(`/api/example-workbooks/seismic-pra-bundle?example=${encodeURIComponent(selected)}`), fetchJson<SeismicPraExampleOption[]>("/api/example-workbooks/seismic-pra-examples")]).then(([response, options]) => { if (!cancelled) { setMef(response.seismicPra.mef); setExamples(options); } }).catch((err: unknown) => { if (!cancelled) setError((err as { message?: string }).message ?? "Could not load the Seismic PRA example"); }); return () => { cancelled = true; }; }, [selected]);
