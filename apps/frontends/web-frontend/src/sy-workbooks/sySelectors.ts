...
function filterConformance(sy: SystemsAnalysis, ccId: string, stage: Stage): ConformanceItem[] {
  const stageKey = stage === "operational" ? "operational" : "pre_operational";
  const statusBySr = new Map<string, string>();
  for (const entry of sy.conformanceMatrix) {
...
function ccScore(sy: SystemsAnalysis, ccId: string, stage: Stage): CcScore {
  const items = filterConformance(sy, ccId, stage);
...
function commentsView(sy: SystemsAnalysis, now: Date = new Date()): CommentView[] {
  const reviewers = sy.metadata.reviewers;
  return sy.internalReviewComments.comments.map((c) => {
    ...
    return {
      ...
      section: item?.section ?? "Document (HLR-SY-C)",
...
function stepsFromMef(sy: SystemsAnalysis, persona: SyPersona): SyStep[] {
  ...
  return base.map((s) => {
    switch (s.id) {
      ...
      case "models": return { ...s, status: status(modelsComplete) };
      case "failures": return { ...s, status: status(failuresComplete) };
      case "ccf": return { ...s, status: status(ccfComplete) };
      case "deps": return { ...s, status: status(depsComplete) };
      case "integrity": return { ...s, status: status(integrityComplete) };
