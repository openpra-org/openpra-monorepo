...
const WORKBOOK_DIRECTORIES = [
  "pos-workbooks", "ie-workbooks", "es-workbooks", "sc-workbooks", "sy-workbooks", "hr-workbooks", "da-workbooks",
  "esq-workbooks", "ms-workbooks", "rc-workbooks", "ri-workbooks", "seismic-pra-workbooks",
  "internal-flood-pra-workbooks", "internal-fire-pra-workbooks",
];
...
describe("workbook cue content", () => {
  it("keeps only the workflow contribution, analyst action, and a realistic example", () => {
    ...
    expect(cue).toBe("Defines the physical areas used to organize flood sources, propagation paths, exposed SSCs, and scenarios. Record each hydraulically distinct room or connected area. For example, Auxiliary Building, elevation 100 ft, Rooms A-101 and A-102.");
  ...
  it("replaces legacy downstream explanations with the section contribution", () => {
    const cue = String(composeWorkbookCue(
      "SEISMIC",
  ...
  it("keeps both regular and compact subsection cues interactive", async () => {
    render(<>
      <WorkbookSectionHeading workbook="SC" title="Success criteria table" />
      <WorkbookCueLabel workbook="SC" title="Required capacities" className="essec" />
    </>);
...
describe("workbook cue coverage", () => {
  ...
  it.each(WORKBOOK_DIRECTORIES)("covers analytical headers in %s", (directory) => {
    ...
    const rawHeader = /<h(?:1|3)\b[^>]*className="(?:posmain__title|poscard__title|posgen__readout-h|sinlineeditor__title|sstructured__section-title)"[^>]*>/g;
    const uncued = sources.flatMap((source) => Array.from(source.matchAll(rawHeader)).filter((match) => {
