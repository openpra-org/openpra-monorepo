...
interface FaultTreeCutSetLiteral {
  /** Saved basic-event UUID or an event generated by PRAXIS during CCF expansion. */
  basicEventId: string;
  negated: boolean;
}
...
const FaultTreeResultEventIdSchema = z.string().trim().min(1, "Result event id is required");
...
const FaultTreeCutSetLiteralSchema = z.object({
  basicEventId: FaultTreeResultEventIdSchema,
  negated: z.boolean(),
}).strict();
