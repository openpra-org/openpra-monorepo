...
import { sameWeatherSite, weatherIsReviewed, weatherIssues, weatherSourceDistance, windToward } from "../weather";
...
describe("MACCS weather and MacMetGen configuration", () => {
  ...
  it("requires location review and invalidates it when the shared site moves", () => {
    const c = parseRcWeatherConfiguration(config), site = { latitude: 35.31028, longitude: -93.23194 };
    const w: RcWeatherInputs = { revision: 1, settings: { latitude: c.latitude, longitude: c.longitude, year: 2020, windSectors: 64 }, data: parseRcWeather(weather).data, configuration: c };
