...
export class ProducerService implements OnApplicationBootstrap, OnApplicationShutdown {
    ...
    constructor(private readonly queueService: QueueService, private readonly rabbitmqService: RabbitMQChannelModelService, private readonly queueConfigFactory: QueueConfigFactory, private readonly minioService: MinioService, private readonly sequenceExtractorService: SequenceExtractorService) {
    ...
    public async createAndQueueAdaptiveSequenceBatch(quantRequest: NodeQuantRequest): Promise<string[]> {
        const parentJobId = uuidv4();
        const { sequenceRequests, sequenceJobIds } = this.sequenceExtractorService.extractSequenceRequests(quantRequest, parentJobId);
