...
describe('SequenceExtractorService', () => {
    ...
    it('should throw error if no event trees found', () => {
        ...
        expect(() => service.extractSequenceRequests(request, 'parent-id')).toThrow('No event trees found in the model');
    });
    it('should extract sequence requests correctly', () => {
        ...
        const seq1Request = result.sequenceRequests.find((r) => r._id === 'parent-id-SEQ1');
        ...
        expect(seq1Request?.model?.eventTrees?.[0].sequences?.[0].name).toBe('SEQ1');
