    ...
    public extractSequenceRequests(originalRequest: NodeQuantRequest, parentJobId: string): SequenceExtractionResult {
        ...
        for (const eventTree of originalRequest.model.eventTrees) {
            ...
            for (const sequence of eventTree.sequences) {
                ...
                const sequenceRequest = this.createSequenceRequest(originalRequest, eventTree.name, String(sequence.name), sequenceJobId);
                sequenceRequests.push(sequenceRequest);
            }
        }
        this.logger.debug(`Extracted ${sequenceRequests.length} sequence requests from parent job ${parentJobId}`);
        return {
            sequenceRequests,
            sequenceJobIds,
        };
    }
    private createSequenceRequest(originalRequest: NodeQuantRequest, eventTreeName: string, sequenceName: string, sequenceJobId: string): NodeQuantRequest {
