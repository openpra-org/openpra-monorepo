...
describe('runQuantificationWithWorker', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        mockWorker = Object.assign(new EventEmitter(), {
            terminate: vi.fn().mockResolvedValue(undefined),
            removeAllListeners: vi.fn(),
        });
        (Worker as unknown as ReturnType<typeof vi.fn>).mockImplementation(function () {
            return mockWorker;
        });
    });
    it('should resolve with result on message', async () => {
        const promise = runQuantificationWithWorker({});
