...
double ImportanceAnalyzer<Calculator>::CalculateMif(int index)  {
  ...
  auto p_conditional = [index, this](bool state) {
    p_vars_[index] = state;
    auto *analyzer = static_cast<ProbabilityAnalyzer<Calculator>*>(prob_analyzer());
    const double probability = analyzer->CalculateTotalProbability(p_vars_);
    return prob_analyzer()->ApplyInitiatingEventFrequency(probability);
  };
