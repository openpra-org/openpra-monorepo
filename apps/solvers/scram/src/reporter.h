...
class Reporter {
 public:
  ...
  /// @throws IOError  The write operation has failed.
    void Report(const core::RiskAnalysis& risk_an, std::FILE* out,
                            bool indent = true,
                            std::optional<std::uint64_t> total_runtime_clock_start = std::nullopt);
 ...
 private:
  ...
  /// @param[in] total_runtime_clock_start  Optional start time for total runtime calculation.
  /// @param[in] report_times  Optional map of per-result report generation times.
    void ReportInformation(const core::RiskAnalysis& risk_an,
                                                 ...
                                                 const std::unordered_map<const void*, double>* report_times = nullptr);
