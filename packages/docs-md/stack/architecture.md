...
```text
openpra-monorepo/
├── apps/                 # Current applications, interfaces, solvers, and utilities
...
├── packages/docs-md/     # Unified documentation site and documentation tooling
├── packages/             # Components retained while the apps-based migration continues
...
- Reusable and legacy editor code remains under `packages/frontend` until it is migrated into the apps-based architecture.
...
The documentation site is built from `packages/docs-md`. API reference material is generated during CI from TypeScript, Rust, C++, and Python sources; generated output is not maintained as a second documentation application.
