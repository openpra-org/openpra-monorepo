...
impl<'a> ImportanceAnalysis<'a> {
    ...
    /// Calculates Fussell-Vesely importance using BDD-based cut set extraction
    ///
    /// Fussell-Vesely importance measures the contribution of an event to the
    /// top event probability by computing:
    ///
    /// FV_i = Σ(P(cut sets containing event i)) / P(top event)
    ///
    /// This quantifies the probability that event i is involved in a system
    /// failure scenario, given that the system has failed.
    ///
    /// # Algorithm
    /// 1. Convert fault tree to BDD
    /// 2. Extract all minimal cut sets
    /// 3. For each event, identify cut sets containing it
    /// 4. Calculate probability of union of those cut sets
    /// 5. Divide by top event probability
    ///
    /// # Returns
    ...
    /// use praxis::analysis::importance::ImportanceAnalysis;
    ///
    /// let importance = ImportanceAnalysis::new(&fault_tree, top_prob).unwrap();
    /// let fv_values = importance.compute_fussell_vesely_from_bdd().unwrap();
    ///
    /// for (event_id, fv) in fv_values {
    ///     println!("{}: FV = {:.4}", event_id, fv);
    /// }
    /// ```
    pub fn compute_fussell_vesely_from_bdd(&self) -> Result<HashMap<String, f64>> {
        // Convert fault tree to BDD
        let mut bdd = Bdd::new();
        let root = bdd.from_fault_tree(self.fault_tree)?;

        // Extract minimal cut sets
        let cut_sets = bdd.extract_cut_sets(root);

        // Calculate FV for each event
        self.compute_fussell_vesely_from_cutsets(&cut_sets)
    }
...
mod tests {
    ...
    #[test]
    fn test_fussell_vesely_from_bdd_or_gate() {
        // For OR gate (E1 OR E2):
        // Cut sets: {E1}, {E2}
        // FV(E1) = P(E1) / P(E1 OR E2) = 0.1 / 0.28 = 0.357
        // FV(E2) = P(E2) / P(E1 OR E2) = 0.2 / 0.28 = 0.714
        ...
        // Check that we have FV for both events
        assert_eq!(fv_values.len(), 2);

        let fv_e1 = fv_values.get("E1").unwrap();
        let fv_e2 = fv_values.get("E2").unwrap();

        // P(E1 OR E2) = 0.1 + 0.2 - 0.1*0.2 = 0.28
        // FV(E1) = 0.1 / 0.28 ≈ 0.357
        // FV(E2) = 0.2 / 0.28 ≈ 0.714
        assert!((*fv_e1 - 0.357).abs() < 0.01, "FV(E1) = {}", fv_e1);
        assert!((*fv_e2 - 0.714).abs() < 0.01, "FV(E2) = {}", fv_e2);

        // Sum should be close to 1.0 for OR gate
        let sum = fv_e1 + fv_e2;
        assert!((sum - 1.071).abs() < 0.01, "Sum = {}", sum);
    }

    #[test]
    fn test_fussell_vesely_from_bdd_and_gate() {
        // For AND gate (E1 AND E2):
        // Cut sets: {E1, E2}
        // FV(E1) = P(E1 AND E2) / P(E1 AND E2) = 1.0
        // FV(E2) = P(E1 AND E2) / P(E1 AND E2) = 1.0
        let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
        let mut gate = Gate::new("G1".to_string(), Formula::And).unwrap();
        gate.add_operand("E1".to_string());
        gate.add_operand("E2".to_string());
        ...
        // Both events must fail for AND gate, so both have FV = 1.0
        assert!((*fv_e1 - 1.0).abs() < 0.01, "FV(E1) = {}", fv_e1);
        assert!((*fv_e2 - 1.0).abs() < 0.01, "FV(E2) = {}", fv_e2);
    }

    #[test]
    fn test_fussell_vesely_complex_tree() {
        // Tree: TOP = G1 OR E3, where G1 = E1 AND E2
        // Cut sets: {E1, E2}, {E3}
        // FV(E1) = P(E1 AND E2) / P(TOP)
        // FV(E2) = P(E1 AND E2) / P(TOP)
        // FV(E3) = P(E3) / P(TOP)
        let mut ft = FaultTree::new("TestFT".to_string(), "TOP".to_string()).unwrap();
        ...
        // P(G1) = 0.1 * 0.2 = 0.02
        // P(TOP) = P(G1 OR E3) = 0.02 + 0.3 - 0.02*0.3 = 0.314
        // FV(E1) = FV(E2) = 0.02 / 0.314 ≈ 0.064
        // FV(E3) = 0.3 / 0.314 ≈ 0.955

        let fv_e1 = fv_values.get("E1").unwrap();
        let fv_e2 = fv_values.get("E2").unwrap();
        let fv_e3 = fv_values.get("E3").unwrap();

        assert!((*fv_e1 - 0.064).abs() < 0.01, "FV(E1) = {}", fv_e1);
        assert!((*fv_e2 - 0.064).abs() < 0.01, "FV(E2) = {}", fv_e2);
        assert!((*fv_e3 - 0.955).abs() < 0.01, "FV(E3) = {}", fv_e3);

        // E3 is clearly more important (higher FV)
        assert!(fv_e3 > fv_e1);
        assert!(fv_e3 > fv_e2);
    }

    #[test]
    fn test_fussell_vesely_zero_probability() {
        let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
        let mut gate = Gate::new("G1".to_string(), Formula::And).unwrap();
        gate.add_operand("E1".to_string());
        gate.add_operand("E2".to_string());
        ...
        // With zero top probability, all FV values should be 0
        for (_event_id, fv) in fv_values {
            assert_eq!(fv, 0.0);
        }
    }

    #[test]
    fn test_fussell_vesely_bounds() {
        // FV values should always be in [0, 1]
        let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
        let mut gate = Gate::new("G1".to_string(), Formula::Or).unwrap();
        gate.add_operand("E1".to_string());
        gate.add_operand("E2".to_string());
        ...
        ft.add_gate(gate).unwrap();
        ft.add_basic_event(BasicEvent::new("E1".to_string(), 0.9).unwrap())
            .unwrap();
        ft.add_basic_event(BasicEvent::new("E2".to_string(), 0.8).unwrap())
            .unwrap();
        ft.add_basic_event(BasicEvent::new("E3".to_string(), 0.7).unwrap())
            .unwrap();
        ...
        for (_event_id, fv) in fv_values {
            assert!((0.0..=1.0).contains(&fv), "FV = {} out of bounds", fv);
        }
    }

    #[test]
    fn test_fussell_vesely_single_event() {
        // Single event tree - FV should be 1.0
        let mut ft = FaultTree::new("TestFT".to_string(), "E1".to_string()).unwrap();
        ft.add_basic_event(BasicEvent::new("E1".to_string(), 0.5).unwrap())
            .unwrap();
