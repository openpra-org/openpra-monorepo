...
use crate::openpra_mef::fault_tree_quantification::MefFaultTreeGraph;
...
fn mef_node_attrs(node_type: &str, label: &str, k: Option<usize>) -> String {
    match node_type {
        "AND_GATE" | "INHIBIT_GATE" => format!("shape=triangle, label=\"AND\\n{}\"", label),
        "OR_GATE" => format!("shape=invtriangle, label=\"OR\\n{}\"", label),
        "NOT_GATE" => format!("shape=diamond, label=\"NOT\\n{}\"", label),
        "ATLEAST_GATE" => format!(
            "shape=hexagon, label=\">={}/N\\n{}\"",
            k.unwrap_or(2),
            label
        ),
        "BASIC_EVENT" => format!("shape=circle, label=\"{}\"", label),
        "HOUSE_EVENT" => format!("shape=house, label=\"{}\"", label),
        _ => format!("shape=ellipse, label=\"{}\"", label),
    }
}
...
pub fn generate_dot(graph: &MefFaultTreeGraph) -> String {
    let mut reachable: HashSet<String> = HashSet::new();
    let mut queue: std::collections::VecDeque<String> = std::collections::VecDeque::new();
    queue.push_back(graph.top_event_id.clone());
    ...
    for uuid in &reachable {
        let Some(node) = graph.nodes.get(uuid) else { continue };
        let label = if node.name.is_empty() { uuid } else { &node.name };
        let escaped = label.replace('"', "\\\"");
        let attrs = mef_node_attrs(&node.node_type, &escaped, node.k_value);
        dot.push_str(&format!("  \"{}\" [{}];\n", uuid, attrs));

        for child in &node.inputs {
            dot.push_str(&format!("  \"{}\" -> \"{}\";\n", uuid, child));
            if !declared.contains(child.as_str()) && !reachable.contains(child) {
                unresolved.insert(child.clone());
            }
        }
    }
