/// Integration tests for importance analysis
///
/// Tests importance measures (FV, RAW, RRW, Birnbaum) with real fault trees
...
fn test_importance_or_gate_comprehensive() {
    // Test all importance measures for simple OR gate
    // System fails if E1 OR E2 fails
...
fn test_importance_and_gate_critical_components() {
    // Test importance for AND gate where both components are critical
    let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
    ...
    // E2 has higher probability, so E1 is more critical
    ...
    // RRW should be infinite (single points of failure)
    let rrw_values = importance.compute_rrw().unwrap();
    let rrw_e1 = rrw_values.get("E1").unwrap();
    let rrw_e2 = rrw_values.get("E2").unwrap();
...
fn test_importance_complex_tree_ranking() {
    // Complex tree: (E1 AND E2) OR E3
    // Should rank events by importance
...
fn test_importance_redundant_events() {
    // Test with redundant configuration: E1 OR E2 OR E3
    // All events should have similar importance
    let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
    let mut gate = Gate::new("G1".to_string(), Formula::Or).unwrap();
    gate.add_operand("E1".to_string());
    gate.add_operand("E2".to_string());
    ...
    // All events should have similar RRW (removing any reduces risk similarly)
    let rrw_values = importance.compute_rrw().unwrap();
    let rrw_e1 = rrw_values.get("E1").unwrap();
    let rrw_e2 = rrw_values.get("E2").unwrap();
...
fn test_importance_consistency_across_measures() {
    ...
    // All measures should agree on which event is more important
    let fv_e1 = fv_values.get("E1").unwrap();
    let fv_e2 = fv_values.get("E2").unwrap();
    let bi_e1 = bi_values.get("E1").unwrap();
    let bi_e2 = bi_values.get("E2").unwrap();

    // E2 has higher probability, should have higher FV and Birnbaum
    if *fv_e2 > *fv_e1 {
        assert!(
            *bi_e2 > *bi_e1,
    ...
    // All RAW and RRW values should be >= 1
...
fn test_importance_all_measures_present() {
    // Verify that all importance measures can be computed together
    let mut ft = FaultTree::new("TestFT".to_string(), "G1".to_string()).unwrap();
    let mut gate = Gate::new("G1".to_string(), Formula::And).unwrap();
    gate.add_operand("E1".to_string());
    gate.add_operand("E2".to_string());
    ...
    // Compute all measures - should not fail
    let fv_result = importance.compute_fussell_vesely_from_bdd();
    ...
    // All should have values for both events
