...
/// @throws std::exception  All other problems.
    void RunScram(const po::variables_map& vm) {
        double settings_time = omp_get_wtime();
        ...
        std::cout<<"--> Settings elapsed time= "<<(omp_get_wtime()-settings_time)*1000<<" ms"<<std::endl;
        double process_input_files_time = omp_get_wtime();
        // Process input files
        ...
        std::cout<<"--> Input file processing elapsed time= "<<(omp_get_wtime()-process_input_files_time)*1000<<" ms"<<std::endl;
        double risk_analysis_time = omp_get_wtime();
        // Initiate risk analysis with the given information.
        scram::core::RiskAnalysis analysis(model.get(), settings);
        analysis.Analyze();
        std::cout<<"--> Risk analysis elapsed time= "<<(omp_get_wtime()- risk_analysis_time)*1000<<" ms"<<std::endl;
#ifndef NDEBUG
        if (vm.count("no-report") || vm.count("preprocessor") || vm.count("print"))
            return;
#endif
        double reporter_time = omp_get_wtime();
        ...
        std::cout<<"--> Reporter elapsed time= "<<(omp_get_wtime()-reporter_time)*1000<<" ms"<<std::endl;
    }
...
int main(int argc, char* argv[]) {
    double whole_process = omp_get_wtime();
    double parsing_time = omp_get_wtime(); //measuring the elapsed time
    LIBXML_TEST_VERSION
    xmlInitParser();
    ...
    try {
        ...
        std::cout<<"--> Parsing time= "<<(omp_get_wtime()-parsing_time)*1000<<" ms"<<std::endl;
    ...
    std::cout<<"-> Whole process elapsed time= "<<(omp_get_wtime()-whole_process)*1000<<" ms"<<std::endl;
}  // End of main.
