...
# BDD test script with various parameter combinations
# Tests:
#   - BDD alone with --limit-order 4, 5, 6
#   - BDD + --probability with --limit-order 4, 5, 6
#   - BDD + --prime-implicants with --limit-order 4, 5, 6
#   - BDD + --probability + --prime-implicants with --limit-order 4, 5, 6
# Total: 4 configurations × 3 limit-orders × 2 input files = 24 tests

set -e

# Directories
INPUT_DIR="./input"
OUTPUT_DIR="./output/bdd"
...
echo "SCRAM CLI BDD Test Results" > "$RESULTS_LOG"
echo "==========================" >> "$RESULTS_LOG"
echo "Date: $(date)" >> "$RESULTS_LOG"
echo "" >> "$RESULTS_LOG"
echo "Test Configurations:" >> "$RESULTS_LOG"
echo "  1. BDD only" >> "$RESULTS_LOG"
echo "  2. BDD + --probability" >> "$RESULTS_LOG"
echo "  3. BDD + --prime-implicants" >> "$RESULTS_LOG"
echo "  4. BDD + --probability + --prime-implicants" >> "$RESULTS_LOG"
echo "" >> "$RESULTS_LOG"
...
run_test() {
    local input_file=$1
    local limit_order=$2
    local test_type=$3  # "bdd-only", "probability", "prime-implicants", or "prob-prime"
    
    local test_name="bdd_${test_type}_${input_file%.xml}_lo${limit_order}"
    ...
    echo "Test $TEST_NUM: BDD - ${test_type}" | tee -a "$RESULTS_LOG"
    echo "========================================" | tee -a "$RESULTS_LOG"
    ...
    EXIT_CODE=0
    if [ "$test_type" = "bdd-only" ]; then
        CMD="scram-cli --bdd --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: BDD only" | tee -a "$RESULTS_LOG"
        scram-cli --bdd \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
    elif [ "$test_type" = "probability" ]; then
        CMD="scram-cli --bdd --probability --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: BDD + probability" | tee -a "$RESULTS_LOG"
        scram-cli --bdd --probability \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
    elif [ "$test_type" = "prime-implicants" ]; then
        CMD="scram-cli --bdd --prime-implicants --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: BDD + prime-implicants" | tee -a "$RESULTS_LOG"
        scram-cli --bdd --prime-implicants \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
    elif [ "$test_type" = "prob-prime" ]; then
        CMD="scram-cli --bdd --probability --prime-implicants --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: BDD + probability + prime-implicants" | tee -a "$RESULTS_LOG"
        scram-cli --bdd --probability --prime-implicants \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
...
# Test types
TEST_TYPES=("bdd-only" "probability" "prime-implicants" "prob-prime")
...
echo "  Test Types: ${#TEST_TYPES[@]} (bdd-only, probability, prime-implicants, prob-prime)" | tee -a "$RESULTS_LOG"
