...
# ZBDD test script with various parameter combinations
# Tests:
#   - ZBDD alone with --limit-order 4, 5, 6
#   - ZBDD + --probability + --rare-event with --limit-order 4, 5, 6
#   - ZBDD + --probability + --mcub with --limit-order 4, 5, 6
...
echo "SCRAM CLI ZBDD Test Results" > "$RESULTS_LOG"
echo "===========================" >> "$RESULTS_LOG"
echo "Date: $(date)" >> "$RESULTS_LOG"
echo "" >> "$RESULTS_LOG"
echo "Test Configurations:" >> "$RESULTS_LOG"
echo "  1. ZBDD only" >> "$RESULTS_LOG"
echo "  2. ZBDD + --probability + --rare-event" >> "$RESULTS_LOG"
echo "  3. ZBDD + --probability + --mcub" >> "$RESULTS_LOG"
echo "" >> "$RESULTS_LOG"
...
run_test() {
    local input_file=$1
    local limit_order=$2
    local test_type=$3  # "zbdd-only", "rare-event", or "mcub"
    
    local test_name="zbdd_${test_type}_${input_file%.xml}_lo${limit_order}"
    ...
    echo "Test $TEST_NUM: ZBDD - ${test_type}" | tee -a "$RESULTS_LOG"
    echo "========================================" | tee -a "$RESULTS_LOG"
    ...
    EXIT_CODE=0
    if [ "$test_type" = "zbdd-only" ]; then
        CMD="scram-cli --zbdd --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: ZBDD only" | tee -a "$RESULTS_LOG"
        scram-cli --zbdd \
        ...
        CMD="scram-cli --zbdd --probability --rare-event --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: ZBDD + probability + rare-event" | tee -a "$RESULTS_LOG"
        scram-cli --zbdd --probability --rare-event \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
    elif [ "$test_type" = "mcub" ]; then
        CMD="scram-cli --zbdd --probability --mcub --limit-order $limit_order $INPUT_DIR/$input_file --output $output_file"
        echo "  Mode: ZBDD + probability + mcub" | tee -a "$RESULTS_LOG"
        scram-cli --zbdd --probability --mcub \
            --limit-order $limit_order \
            "$INPUT_DIR/$input_file" \
            --output "$output_file" \
            > "$stdout_file" 2> "$stderr_file" || EXIT_CODE=$?
...
# Main test execution
echo "Starting ZBDD Tests..."
echo ""

# Test types
TEST_TYPES=("zbdd-only" "rare-event" "mcub")
...
echo "  Test Types: ${#TEST_TYPES[@]} (zbdd-only, rare-event, mcub)" | tee -a "$RESULTS_LOG"
