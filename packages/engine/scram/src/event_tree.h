...
class InitiatingEvent : public Element, public Usage {
 public:
  ...
  /// Sets the frequency expression for the initiating event.
  void frequency(Expression* expression) { frequency_ = expression; }

  /// @returns true if a frequency expression has been provided.
  bool HasFrequency() const {
    return frequency_ != nullptr || GetAttribute("frequency") != nullptr;
  }

  /// @returns The frequency expression associated with this initiating event.
  ///
  /// @pre HasFrequency() == true.
  Expression& frequency_expression() const {
    assert(frequency_ && "Initiating event frequency is not set.");
    return *frequency_;
  }

  /// @returns The numeric value of the initiating event frequency.
  double frequency_value() const {
    if (frequency_)
      return frequency_->value();
    if (const Attribute* attr = GetAttribute("frequency")) {
      try {
  ...
  /// @returns The optional frequency expression pointer.
  Expression* frequency_expression_ptr() const { return frequency_; }

 private:
  EventTree* event_tree_ = nullptr;  ///< The optional event tree specification.
  Expression* frequency_ = nullptr;  ///< Optional frequency expression.
};
