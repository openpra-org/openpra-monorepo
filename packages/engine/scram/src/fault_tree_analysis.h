...
/// Summary information about generated products.
struct ProductSummary {
  using ProductList = std::vector<std::vector<int>>;

  int product_count = 0;  ///< Products retained after filtering.
  int original_product_count = 0;  ///< Products enumerated before filtering.
  int pruned_products = 0;  ///< Products removed by cut-off.
  std::vector<int> distribution;  ///< Counts grouped by product order.
  std::vector<int> event_indices;  ///< Unique basic event indices.
  bool cut_off_applied = false;  ///< Whether cut-off filtering happened.
  double applied_cut_off = 0.0;  ///< Cut-off value used for filtering.
};
...
class ProductContainer {
  ...
  /// Iterator over stored products supporting optional filtered collections.
  class Iterator
 ...
 public:
  ...
  /// @returns The underlying PDAG used for product indexing.
  ///
  /// This enables serializers/reporters to emit a stable BasicEvent -> index map
  /// for later deserialization of compact cut set encodings.
...
class FaultTreeAnalysis : public Analysis {
 public:
  ...
  ///
  /// @param[in] frequency  The initiating event frequency (default 1.0).
  ///
  /// @pre Must be called before Analyze().
 ...
 private:
  ...
  double initiating_event_frequency_ = 1.0;  ///< Initiating event frequency for event tree sequences.
