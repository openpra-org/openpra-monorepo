...
void Initializer::Define(const xml::Element& xml_node,
  ...
  for (const xml::Element& child : GetNonAttributeElements(xml_node)) {
    if (child.name() == "frequency") {
      if (initiating_event->HasFrequency()) {
        SCRAM_THROW(ValidityError("Initiating event frequency redefined."))
            << boost::errinfo_at_line(child.line());
      }
      auto expressions = GetNonAttributeElements(child);
      auto expr_it = expressions.begin();
      if (expr_it == expressions.end()) {
        SCRAM_THROW(ValidityError("Initiating event frequency must provide exactly one expression."))
            << boost::errinfo_at_line(child.line());
      }
      const xml::Element &expr_node = *expr_it;
      if (++expr_it != expressions.end()) {
        SCRAM_THROW(ValidityError("Initiating event frequency must provide exactly one expression."))
            << boost::errinfo_at_line(child.line());
      }
      initiating_event->frequency(
          GetExpression(expr_node, ""));
    }
  }
}
