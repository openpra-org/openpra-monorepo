...
Gate::Gate(Connective type, Pdag* graph)
    : Node(graph),
      type_(type),
      mark_(false),
      module_(false),
...
template <>
void Gate::AddConstantArg<false>()  {
  switch (type_) {
    case kNull:
    case kAnd:
      MakeConstant(false);
...
void Gate::TransferArg(int index, const GatePtr& recipient)  {
...
void Gate::ShareArg(int index, const GatePtr& recipient)  {
  assert(!constant() && "Improper use case.");
  assert(index != 0);
  assert(args_.count(index));
  if (auto it_g = ext::find(gate_args_, index)) {
...
void Gate::NegateArg(int existing_arg)  {
  assert(!constant() && "Improper use case.");
...
void Gate::CoalesceGate(const GatePtr& arg_gate)  {
  assert(!constant() && "Improper use case.");
...
void Gate::JoinNullGate(int index)  {
  assert(index != 0);
  assert(args_.count(index));
...
void Gate::ProcessConstantArg(const NodePtr& arg, bool state)  {
...
void Gate::MakeConstant(bool state)  {
  assert(!constant());
  EraseArgs();
  type(kNull);
...
void Gate::ProcessDuplicateArg(int index)  {
...
void Gate::ProcessAtleastGateDuplicateArg(int index)  {
...
void Gate::ProcessComplementArg(int index)  {
  assert(type_ != kNot && type_ != kNull);
  assert(args_.count(-index));
...
Pdag::Pdag(const mef::Gate& root, bool ccf, const mef::Model* model)
    : Pdag() {
...
void Pdag::PropagateNullGate(const GatePtr& gate)  {
...
struct GraphLogger {
  ...
  explicit GraphLogger(const GatePtr& gate)  {
  ...
  void GatherInformation(const GatePtr& gate)  {
  ...
  void Log(const GatePtr& gate)  {
    ++gate_types[static_cast<int>(gate->type())];
    if (gate->module())
      ++num_modules;
  ...
  int Count(const std::unordered_set<int>& container)  {
  ...
  int CountComplements(const std::unordered_set<int>& container)  {
  ...
  int CountOverlap(const std::unordered_set<int>& container)  {
    return boost::count_if(container, [&container](int index) {
      return index < 0 && container.count(-index);
    });
  }
...
void Pdag::Log()  {
  ...
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kAnd)])
      << "AND gates: " << logger.gate_types[static_cast<int>(kAnd)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kOr)])
      << "OR gates: " << logger.gate_types[static_cast<int>(kOr)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kAtleast)])
      << "K/N gates: " << logger.gate_types[static_cast<int>(kAtleast)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kXor)])
      << "XOR gates: " << logger.gate_types[static_cast<int>(kXor)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kNot)])
      << "NOT gates: " << logger.gate_types[static_cast<int>(kNot)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kNand)])
      << "NAND gates: " << logger.gate_types[static_cast<int>(kNand)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kNor)])
      << "NOR gates: " << logger.gate_types[static_cast<int>(kNor)];
  BLOG(DEBUG5, logger.gate_types[static_cast<int>(kNull)])
      << "NULL gates: " << logger.gate_types[static_cast<int>(kNull)];
...
#ifdef SCRAM_ENABLE_PDAG_STREAM_OUTPUT
...
#endif  // SCRAM_ENABLE_PDAG_STREAM_OUTPUT
