...
class Pdag : private boost::noncopyable {
 public:
  ...
  /// @returns The initiating event frequency multiplier for this graph.
  double initiating_event_frequency() const { return initiating_event_frequency_; }

  /// Sets the initiating event frequency multiplier.
  /// This is used when analyzing event tree sequences where cut set probabilities
  /// need to be multiplied by the initiating event frequency before comparison with cut-off.
  ///
  /// @param[in] frequency  The initiating event frequency (default 1.0 for standalone fault trees).
  void initiating_event_frequency(double frequency) {
    assert(frequency > 0.0 && "Initiating event frequency must be positive.");
    initiating_event_frequency_ = frequency;
  }
 ...
 private:
  ...
  /// Initiating event frequency multiplier for event tree sequences (1.0 for standalone fault trees)
