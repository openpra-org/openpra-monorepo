...
async function fetchModelList(){
    try {
      return await TypedModelApiManager.getExternalHazards(ApiManager.getCurrentUser().user_id);
    } catch (error) {
...
const getFixtures = async (): Promise<JSX.Element[]> => {
    try {
      ...
      const externalHazardsModelList: ExternalHazardsModel[] = modelList.map((item: any) => {
        return new ExternalHazardsModel(item.id, item.label.name, item.label.description, item.users);
      });
      //now we map these events to what they should be and display them
      const genericListItems = externalHazardsModelList.map((modelItem: ExternalHazardsModel) => (
        <GenericListItem
