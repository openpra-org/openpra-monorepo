...
async function fetchModelList(){
    try {
      return await TypedModelApiManager.getInternalHazards(ApiManager.getCurrentUser().user_id);
    } catch (error) {
...
const getFixtures = async (): Promise<JSX.Element[]> => {
    try {
      ...
      const internalHazardsList: InternalHazardsModel[] = modelList.map((item: any) => {
        return new InternalHazardsModel(item.id, item.label.name, item.label.description, item.users);
      });
      //now we map these events to what they should be and display them
      const genericListItems = internalHazardsList.map((modelItem: InternalHazardsModel) => (
        <GenericListItem
