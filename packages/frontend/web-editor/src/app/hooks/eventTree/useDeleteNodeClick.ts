...
function useDeleteNodeClick(clickedNodeId: NodeProps['id']): () => void {
  ...
  const deleteNode: () => void = () => {
    ...
    const sequenceIdNodes = nodes.filter(
      (node) => node.type === 'outputNode' && node.data.isSequenceId === true,
    );

    // Check if the node we're trying to delete has sequence ID nodes connected to it
    const connectedOutputNodes: string[] =
      getConnectedOutputNodes(clickedNodeId);
    const connectedSequenceIdNodes: string[] = connectedOutputNodes.filter(
      (nodeId) => isSequenceIdNode(nodeId),
    );
    ...
    function getConnectedOutputNodes(nodeId: string): string[] {
      ...
      while (stack.length > 0) {
        ...
        const connectedNodes = edges
          .filter(
            (edge) =>
              edge.source === currentId && isOutputOrInvisibleNode(edge.target),
          )
    ...
    const hasChildren = edges.some(
      (edge) =>
        edge.source === clickedNodeId && !isOutputOrInvisibleNode(edge.target),
    );
    ...
    if (siblingEdges.length === 0) {
      ...
      const updatedEdges = edges.map((edge) => {
        if (
          edge.target === clickedNodeId ||
          connectedOutputNodes.includes(edge.target)
        ) {
