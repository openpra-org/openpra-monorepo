...
export function EditableTable(): JSX.Element | null {
  ...
  useEffect(() => {
    console.log(params.fmeaId);
    const loadFmea = async (): Promise<void> => {
      const res = await FmeaApiManager.getFmea(Number(params.fmeaId), {
        title: "title",
  ...
  function updateCell(id: string, column: string, value: string): void {
    const update = async (): Promise<void> => {
      const res = await FmeaApiManager.updateCell(Number(params.fmeaId), {
        rowId: id,
        column,
        value,
      });
  ...
  function deleteRows() {
    const del = async (): Promise<void> => {
      ...
      const res = await FmeaApiManager.deleteRows(Number(params.fmeaId), {
        rowIds: rowIds,
      });
      //console.log(res);
  ...
  function deleteRow(id: string): void {
    const del = async (): Promise<void> => {
      const res = await FmeaApiManager.deleteRow(Number(params.fmeaId), id);
      //console.log(res);
  ...
  const addNewRow = (): void => {
    const callAddRow = async (): Promise<void> => {
      await FmeaApiManager.addRow(Number(params.fmeaId)).then((resposne) => {
      ...
      if (
        ...
        if (editingColumn) {
          ...
          const addColumn = async (): Promise<void> => {
            ...
            //console.log(body);
            await FmeaApiManager.addColumn(Number(params.fmeaId), body).then(
              (res) => {
                //console.log(res);
                res.columns.push({
                  id: "actions",
