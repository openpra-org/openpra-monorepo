...
test.describe("event sequence test", () => {
  ...
  test("creates event sequence diagram", async ({ page }) => {
    ...
    await page
      .getByRole("button")
      .filter({ hasText: "Event Sequence Diagrams" })
      .click();
    ...
    await page
      ...
      .click();
    await page
      .getByLabel("Event Sequence Diagram Name")
      .fill(eventSequenceDiagramName);
    await page.getByRole("button", { name: "Create", exact: true }).click();
    ...
    // expect
    await expect(
      page.locator("a").filter({ hasText: eventSequenceDiagramName }),
    ).toHaveText(eventSequenceDiagramName);
  });
  ...
  test("opens editor and has default structure", async ({ page }) => {
    ...
    await page
      .locator("a")
      .filter({ hasText: eventSequenceDiagramName })
      .click();

    // expect
    await expect(page.getByTestId("initiating-event-node")).toContainText(
      "Initiating Event",
    );
  });
});
