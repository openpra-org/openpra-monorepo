...
test.describe("Fmea test", () => {
  ...
  test("displays internal events page", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button").filter({ hasText: "Initiating Events" }),
    ).toHaveText("Initiating Events");
  });
  ...
  test("creates initiating event", async ({ page }) => {
    ...
    await page
      .getByRole("button")
      .filter({ hasText: "Initiating Events" })
      .click();

    const initiatingEventName =
      "Initiating Event" + Math.floor(Math.random() * 1000);

    await page
      .getByRole("button")
      .filter({ hasText: "Create Initiating Event" })
      .click();
    ...
    // expect
    await expect(
      page.getByRole("button").filter({ hasText: "Add Row" }).first(),
    ).toHaveText("Add Row");
  });
  ...
  test("Add New Row", async ({ page }) => {
    ...
    // expect
    await expect(
      page.locator('div[data-grid-visible-row-index="0"]'),
    ).toBeVisible();
  });
  ...
  test("Add New Column Modal", async ({ page }) => {
    ...
    // expect
    await expect(
      page.locator("h1").filter({ hasText: "Add a New Column" }),
    ).toHaveText("Add a New Column");
  });
  ...
  test("Add New Column to datagrid", async ({ page }) => {
    ...
    // expect
    await expect(
      page.locator("(//div[text()='Test Column'])[1]"),
    ).toBeVisible();
  });
  ...
  test("Edit a row text cell", async ({ page }) => {
    ...
    await page
      .locator(
        'div[data-grid-row-index="0"] >> div[data-testid="Test Column For Edit"]',
      )
      .nth(1)
      .fill("Testing Text");
    await page
  ...
  test("Edit a row dropdown cell", async ({ page }) => {
    ...
    await page
      .locator(".euiModalBody .euiSelect.euiFormControlLayout--1icons")
      .nth(0)
  ...
  test("Edit New Column Modal", async ({ page }) => {
    ...
    await page
      ...
      .click();
    await page
      .locator("(//div[@title='Test Column For Edit']//button)[1]")
      .click();

    // expect
    await expect(
      page.locator("h1").filter({ hasText: "Edit Column" }),
    ).toHaveText("Edit Column");
  });
});
