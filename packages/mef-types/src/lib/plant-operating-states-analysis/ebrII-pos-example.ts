...
import {
  ...
  InitiatingEvent,
  FrequencyUnit,
} from '../core/events'; // Assuming path
import {
  ImportanceLevel,
  ScreeningStatus,
  SuccessCriteriaId,
} from '../core/shared-patterns'; // Assuming path - Enums are likely here
...
function getFrequencyValue(
  frequency: Frequency | FrequencyWithDistribution | undefined,
): number {
...
const spentFuelSource: RadioactiveSource = {
  ...
  location: 'IN_VESSEL',
  description:
    'Storage basket for spent fuel assemblies within the primary tank',
  radionuclides: ['Cs-137', 'Sr-90', 'Pu-239', 'I-131'],
  status: 'Stored',
};
...
const activatedSteelSource: RadioactiveSource = {
  ...
  location: 'IN_VESSEL',
  description:
    'Activated structural steel components within the primary system',
...
const fullPowerPOS = {
  ...
  detailedRadioactiveSources: [
    coreSource,
    spentFuelSource,
    activatedSodiumSource,
    activatedSteelSource,
  ],
  // ... other properties
  ...
  riskSignificance: {
    ...
    importanceMeasures: [
      {
        componentId: controlRodBank1Ref.componentId,
        fussellVesely: 0.15,
        RAW: 100,
        RRW: 1.2,
      },
      {
        componentId: safetyRodRef.componentId,
        fussellVesely: 0.05,
        RAW: 80,
        RRW: 1.1,
      },
      {
        componentId: sdCoolerARef.componentId,
        fussellVesely: 0.1,
        RAW: 50,
        RRW: 1.1,
      },
  ...
  plantRepresentationAccuracy: {
    ...
    sufficiencyJustification:
      'Key safety systems and passive features well-characterized.',
  },
...
const ebr2HazardousSources: HazardousSources = {
  ...
  operatingStatesFrequencyDuration: {
    ...
    tripHistory: [
      {
        date: '1989-05-15',
        description: 'Manual Scram due to BOP fault signal',
      },
      {
        date: '1989-08-22',
        description: 'Automatic Scram on high flux channel',
      },
    ],
    summary: [
      {
        ...
        averageDuration:
          fullPowerPOS.meanDuration /
          getFrequencyValue(fullPowerPOS.meanFrequency),
...
const lpsdGroup: PlantOperatingStatesGroup = {
  ...
  plantOperatingStateIds: [
    controlledShutdownPOS.uuid as string /* Add other POS UUIDs */,
  ],
  groupingJustification: 'Similar DHR challenges...',
  representativeCharacteristics: [
    'Reactor Subcritical',
    'Decay Heat Removal Critical',
    '...',
  ],
};
...
const posDocumentation = {
  ...
  identificationProcessDetails:
    'Based on plant operating procedures and POS analysis methodology',
  groupingProcessDetails:
    'States grouped by similar thermal-hydraulic conditions',
  screeningCriteriaDocumentation:
    'Standard ASME/ANS screening criteria applied',
  stateFrequencyDocumentation: 'Based on historical plant data from 1985-1990',
  stateDurationDocumentation:
    'Based on historical plant data and operating procedures',
  referencesToSupportingAnalyses: [
    'EBR-II Safety Analysis Report',
    'SHRT Test Series Results',
  ],
  knownIssuesAndLimitations:
    'Limited operational data for some transient evolutions',
  peerReviewFindings: [
    {
  ...
  stateGroupDefinitions:
    'Power Operation, Shutdown, Refueling, and Maintenance groups defined',
  stateCharacteristics:
    'Key characteristics documented for each state including thermal conditions',
  meanDurationsDetails:
    'Statistical analysis of operational data from 1985-1990',
  decayHeatDetails: 'Time-dependent decay heat curves developed for each POS',
  praTaskInterfaces:
    'Interfaces with event sequence and systems analysis documented',
  modelUncertaintySources:
    'Uncertainties in decay heat calculation and transition timing',
  asBuiltLimitations:
    'Pre-operational design information used where as-built details unavailable',
...
const validationRules: POSValidationRules = {
  mutualExclusivityRules: {
    ...
    verificationMethod:
      'Transition matrix analysis and boundary condition verification',
  },
  collectiveExhaustivityRules: {
    ...
    configurationCoverage:
      'All possible plant configurations are mapped to a defined state',
  },
  transitionRules: {
    transitionMatrix: {
    ...
    transitionTriggers: {
      'Full Power to Controlled Shutdown':
        'Manual operation, normal end of cycle',
      'Full Power to Emergency Shutdown':
        'Trip signals, safety system actuation',
    },
  },
};
...
const ebr2_pos_analysis: PlantOperatingStatesAnalysis = {
  ...
  modelUncertainty: [
    // Assume ModelUncertaintyInfo structure is okay
    {
      ...
      treatment:
        'Log-uniform distribution used in quantification; sensitivity study performed.',
      reasonableAlternatives: [
        'Use generic data only',
        'Use adjacent site data',
      ],
    },
  ],
  ...
  plantRepresentationAccuracy: {
    ...
    improvementPlans: [
      'Refine CCF analysis with more detailed component data if available.',
    ],
  },
};
