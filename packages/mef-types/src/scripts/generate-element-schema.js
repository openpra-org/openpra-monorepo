...
if (!validElements.includes(element)) {
  console.error(
    `Invalid element name. Valid elements are: ${validElements.join(', ')}`,
  );
  process.exit(1);
}
...
function handlePatternTypes(schema, elementDir) {
  ...
  try {
    ...
    const tsFiles = findTsFiles(elementDir).map((file) =>
        ...
        for (const match of patternMatches) {
          ...
          if (!pattern) {
            ...
            if (typeRef) {
              ...
              const patternDefMatch = content.match(
                new RegExp(`${refName}\\s*=\\s*["'\`]([^"'\`]+)["'\`]`),
              );
              if (patternDefMatch) {
    ...
    for (const [key, def] of Object.entries(schema.definitions)) {
      ...
      if (patternTypes[key]) {
        schema.definitions[key] = {
          ...
          description:
            def.description || `String matching pattern: ${patternTypes[key]}`,
        };
      }
          ...
          if (propDef.$ref) {
            ...
            if (patternTypes[refType]) {
              def.properties[propKey] = {
                ...
                description:
                  propDef.description ||
                  `String matching pattern: ${patternTypes[refType]}`,
              };
            }
          }
        }
  ...
  if (pattern) {
    if (schema.type === 'string') {
            ...
            if (propPattern) {
              if (value.$ref) {
                ...
                schema.properties[key] = {
                  ...
                  description:
                    value.description ||
                    `String matching pattern: ${propPattern}`,
                };
              } else {
      ...
      if (objPattern) {
        ...
        schema.description =
          schema.description || `String matching pattern: ${objPattern}`;
...
function enhanceAllRecordTypes(schema) {
  ...
  const recordTypes = Object.keys(schema.definitions).filter((key) =>
...
try {
  ...
  if (fs.existsSync(elementDir) && fs.statSync(elementDir).isDirectory()) {
    ...
    console.log(
      `Found ${tsFiles.length} TypeScript files in ${element} directory and subdirectories.`,
    );
      ...
      try {
        ...
        if (fs.existsSync(tempOutputFile)) {
          const fileSchema = JSON.parse(
            fs.readFileSync(tempOutputFile, 'utf8'),
          );

          // Add the definitions to our main schema
          if (
            fileSchema.definitions &&
            Object.keys(fileSchema.definitions).length > 0
          ) {
            console.log(
              `Found ${Object.keys(fileSchema.definitions).length} definitions in ${fileName}`,
            );
            ...
            for (const [key, value] of Object.entries(fileSchema.definitions)) {
              ...
              let enhancedSchema = enhanceSchemaWithPattern(
                value,
  ...
  if (
    !finalSchema.definitions ||
    Object.keys(finalSchema.definitions).length === 0
  ) {
    console.warn(
      `Warning: Generated schema for ${element} appears to be empty.`,
    );
  } else {
    console.log(
