...
const runGetRequestStressTest = (endpoint: string, durationInSeconds: number, maxConcurrency: number): void => {
  describe("Microservice GET Request Stress Test", () => {
    ...
    it(
      "should process GET requests and collect metrics",
      async () => {
        ...
        const taskGenerator = async (): Promise<void> => {
          ...
          try {
            totalRequestsSent++;
            const response = await request(app.getHttpServer()).get(endpoint).set("Accept", "application/json");
...
// Example usage:
runGetRequestStressTest(
  "/api/quantify/scram", // Replace with your GET endpoint
  60, // Test duration in seconds (e.g., 10 minutes)
