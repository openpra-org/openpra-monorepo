...
export class ConsumerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private async handleRegularJob(nodeQuantRequest: NodeQuantRequest): Promise<void> {
    ...
    if (!isFileReference && result && typeof result === "object") {
      ...
      totalSeconds =
        resultObj.runtimeSummary?.totalSeconds || resultObj.results?.runtimeSummary?.totalSeconds || analysisSeconds;
  ...
  private extractAdaptiveMetrics(report: any): {
    ...
    const sumOfProducts =
      Array.isArray(results.sumOfProducts) && results.sumOfProducts.length > 0 ? results.sumOfProducts[0] : undefined;
    const sequence =
      Array.isArray(results.initiatingEvents) && results.initiatingEvents[0]?.sequences ?
