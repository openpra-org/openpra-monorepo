...
describe("StorageService", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [StorageService, { provide: MinioService, useValue: mockMinioService }],
    }).compile();
  ...
  describe("getJobStats", () => {
    ...
    it("should return stats for parent and child jobs", async () => {
      ...
      mockMinioService.getJobMetadata.mockResolvedValueOnce(parentMetadata).mockResolvedValueOnce(childMetadata);
      const result = await service.getJobStats("1");
    ...
    it("should handle errors when fetching child stats", async () => {
      ...
      mockMinioService.getJobMetadata.mockResolvedValueOnce(parentMetadata).mockRejectedValueOnce(new Error("Failed"));
      const result = await service.getJobStats("1");
  ...
  describe("getAggregatedJobOutput", () => {
    ...
    it("should aggregate results for parent job", async () => {
      ...
      mockMinioService.getJobMetadata.mockResolvedValueOnce(parentMetadata).mockResolvedValueOnce(childMetadata);
      mockMinioService.getOutputData.mockResolvedValueOnce(JSON.stringify(childOutput)).mockResolvedValueOnce("{}");
      const result = await service.getAggregatedJobOutput("1");
    ...
    it("should handle missing output for completed child job", async () => {
      ...
      mockMinioService.getJobMetadata.mockResolvedValueOnce(parentMetadata).mockResolvedValueOnce(childMetadata);
      mockMinioService.getOutputData.mockRejectedValueOnce(new Error("Not found")).mockResolvedValueOnce("{}");
      const result = await service.getAggregatedJobOutput("1");
      expect(result.failedJobs).toHaveLength(1);
