...
describe("MinioService", () => {
  ...
  describe("getInputData", () => {
    it("should retrieve input data", async () => {
      ...
      expect(mockMinioClient.listObjects).toHaveBeenCalledWith("input-bucket", "input-test-id-", true);
    });
    it("should throw error if no input found", async () => {
      ...
      mockMinioClient.listObjects.mockReturnValue(mockStream);
      await expect(service.getInputData("nonexistent")).rejects.toThrow("No input data found for ID: nonexistent");
    });
    it("should throw error on retrieval failure", async () => {
      mockMinioClient.listObjects.mockImplementation(() => {
        throw new Error("List error");
      });
      await expect(service.getInputData("test-id")).rejects.toThrow("Failed to retrieve input data: List error");
    });
  });
  ...
  describe("getOutputData", () => {
    ...
    it("should throw error if no output found", async () => {
      ...
      mockMinioClient.listObjects.mockReturnValue(mockStream);
      await expect(service.getOutputData("nonexistent")).rejects.toThrow("No output data found for ID: nonexistent");
    });
  });
  describe("createJobMetadata", () => {
    ...
    it("should throw error on failure", async () => {
      ...
      await expect(service.createJobMetadata("job-123", "input-123")).rejects.toThrow(
        "Failed to create job metadata: Create error",
      );
    });
  });
  describe("getJobMetadata", () => {
    ...
    it("should throw error on retrieval failure", async () => {
      ...
      await expect(service.getJobMetadata("job-123")).rejects.toThrow("Failed to retrieve job metadata: Get error");
    });
  });
  describe("updateJobMetadata", () => {
    ...
    it("should throw error on failure", async () => {
      mockMinioClient.getObject.mockRejectedValue(new Error("Get error"));
      await expect(service.updateJobMetadata("job-123", { status: "running" })).rejects.toThrow(
  ...
  describe("markSequenceCompleted", () => {
    ...
    it("should throw error on failure", async () => {
      ...
      await expect(service.markSequenceCompleted("parent-123", "seq-456")).rejects.toThrow(
        "Failed to mark sequence completion: Mark error",
      );
    });
  });
  ...
  describe("getAllJobMetadata", () => {
    it("should retrieve all job metadata", async () => {
      ...
      mockMinioClient.getObject.mockResolvedValueOnce(mockDataStream1).mockResolvedValueOnce(mockDataStream2);
    ...
    it("should throw error on failure", async () => {
      ...
      await expect(service.getAllJobMetadata()).rejects.toThrow("Failed to retrieve all job metadata: List error");
    });
  });
  describe("deleteInputData", () => {
    it("should delete input data", async () => {
      ...
      expect(mockMinioClient.removeObject).toHaveBeenCalledWith("input-bucket", "input-test-1.json");
    });
    it("should throw error on failure", async () => {
      mockMinioClient.listObjects.mockImplementation(() => {
        throw new Error("List error");
      });
      await expect(service.deleteInputData("test")).rejects.toThrow("Failed to delete input data: List error");
    });
  });
  describe("deleteOutputData", () => {
    it("should delete output data", async () => {
      ...
      expect(mockMinioClient.removeObject).toHaveBeenCalledWith("output-bucket", "output-test-1.json");
    });
    it("should throw error on failure", async () => {
      mockMinioClient.listObjects.mockImplementation(() => {
        throw new Error("List error");
      });
      await expect(service.deleteOutputData("test")).rejects.toThrow("Failed to delete output data: List error");
    });
  });
  describe("isHealthy", () => {
  ...
  describe("cleanupOldFiles", () => {
    it("should cleanup old files from all buckets", async () => {
      ...
      expect(mockMinioClient.removeObject).toHaveBeenCalledWith(expect.any(String), "old-file.json");
      expect(mockMinioClient.removeObject).not.toHaveBeenCalledWith(expect.any(String), "recent-file.json");
    });
    it("should throw error on failure", async () => {
      mockMinioClient.listObjects.mockImplementation(() => {
        throw new Error("Cleanup error");
      });
      await expect(service.cleanupOldFiles(7)).rejects.toThrow("Cleanup failed: Cleanup error");
    });
  });
  describe("onModuleInit - error handling", () => {
    ...
    it("should handle BucketAlreadyOwnedByYou error", async () => {
      ...
      mockMinioClient.makeBucket.mockRejectedValueOnce(error).mockRejectedValueOnce(error).mockRejectedValueOnce(error);
      mockMinioClient.bucketExists.mockResolvedValueOnce(true).mockResolvedValueOnce(true).mockResolvedValueOnce(true);
      await service.onModuleInit();
      expect(mockMinioClient.makeBucket).toHaveBeenCalled();
    });
