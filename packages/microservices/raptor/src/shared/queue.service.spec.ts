...
describe("QueueService", () => {
  ...
  describe("setupQueue", () => {
    ...
    it("should throw RpcException if exchange assertion fails", async () => {
      ...
      await expect(service.setupQueue(mockQueueConfig as any, mockChannel as any)).rejects.toThrow(
    ...
    it("should throw RpcException if queue assertion fails", async () => {
      ...
      await expect(service.setupQueue(mockQueueConfig as any, mockChannel as any)).rejects.toThrow(
        `Failed to set up ${mockQueueConfig.name}`,
      );
    });
    ...
    it("should throw RpcException if dead letter setup fails", async () => {
      ...
      await expect(service.setupQueue(mockQueueConfig as any, mockChannel as any)).rejects.toThrow(
        `Failed to set up ${mockQueueConfig.deadLetter.name}`,
      );
    });
  });
});
