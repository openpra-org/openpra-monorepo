/**
 * @file Event.spec.ts
 * @brief Unit tests for the Event class
 * @details This file contains tests for the methods of the Event class.
 */
...
describe('Event', () => {
  ...
  test('addParent should throw an error if the parent is already added', () => {
    const parent = new Event('parent');
    event.addParent(parent);
    expect(() => event.addParent(parent)).toThrowError("Gate is already a parent of this node.");
  });
});
