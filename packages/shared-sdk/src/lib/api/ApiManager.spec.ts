...
describe("ApiManager", () => {
  ...
  it("signInWithUsernameAndPassword throws on non-ok", async () => {
    ...
    await expect(ApiManager.signInWithUsernameAndPassword("user", "bad")).rejects.toThrow("Unauthorized");
  });
  it("signup triggers sign-in on success", async () => {
    ...
    const signInSpy = jest.spyOn(ApiManager, "signInWithUsernameAndPassword").mockResolvedValue(undefined as any);
