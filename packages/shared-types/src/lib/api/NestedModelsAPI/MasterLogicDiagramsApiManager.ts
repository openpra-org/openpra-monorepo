import { Delete, Get, MASTER_LOGIC_DIAGRAM_ENDPOINT, Patch, Post } from "../NestedModelApiManager";
 ...
 */
export async function GetMasterLogicDiagrams(id: string): Promise<NestedModelType[]> {
  try {
    const response = await Get(`${MASTER_LOGIC_DIAGRAM_ENDPOINT}/?id=${id}`);
    return (await response.json()) as Promise<NestedModelType[]>;
  } catch (error) {
    console.error("Failed to fetch master logic diagrams:", error);
    throw error;
  }
}
...
export async function PostMasterLogicDiagram(data: NestedModelJSON, typedModel: string): Promise<NestedModelType> {
  try {
    const response = await Post(`${MASTER_LOGIC_DIAGRAM_ENDPOINT}/`, data, typedModel);
    return (await response.json()) as Promise<NestedModelType>;
  } catch (error) {
    console.error("Failed to post master logic diagram:", error);
    throw error;
  }
}
 ...
 */
export async function PatchMasterlogicDiagramLabel(id: string, data: LabelJSON): Promise<NestedModelType> {
  try {
    const response = await Patch(`${MASTER_LOGIC_DIAGRAM_ENDPOINT}/${id}`, data);
    return (await response.json()) as Promise<NestedModelType>;
  } catch (error) {
    console.error("Failed to patch master logic diagram:", error);
    throw error;
  }
}
 ...
 */
export async function DeleteMasterLogicDiagram(id: string, type: string): Promise<void> {
  try {
    await Delete(`${MASTER_LOGIC_DIAGRAM_ENDPOINT}/?id=${id}&type=${type}`);
  } catch (error) {
    console.error("Failed to delete master logic diagram:", error);
    throw error;
  }
}
