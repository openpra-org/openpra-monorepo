...
export interface SystemBasicEvent extends BasicEvent {
  ...
  /**
   * Failure rate (Lambda) for time-based calculations.
   * Used in SAPHIRE-compatible calculations.
   */
  failureRate?: number;
  
  /**
   * Test interval (Tau) for standby components.
   * Used in SAPHIRE-compatible calculations for periodic testing.
   */
  testInterval?: number;
  
  /**
   * Mission time specific to this basic event.
   * Overrides the system-level mission time if specified.
   */
  missionTime?: number;
  
  /**
   * Calculation method for this basic event.
   * Determines how probability is calculated from other parameters.
   */
  calculationMethod?: "demand" | "standby" | "operating" | "custom";
  
  /**
   * Uncertainty distribution parameters for this basic event.
   */
  uncertaintyDistribution?: {
    ...
    /** Error factor (for lognormal distributions) */
    errorFactor?: number;
  };
  ...
  /**
   * SAPHIRE-specific calculation flags and parameters.
   * Used for direct compatibility with SAPHIRE import/export.
   */
  saphireParameters?: {
    /** Calculation type code used in SAPHIRE */
    calculationType?: string;
    
    /** Failure on demand value (used in some calculation types) */
    failureOnDemand?: number;
    
    /** SAPHIRE-specific flags */
    flags?: string[];
    
    /** Analysis type (e.g., RANDOM) */
    analysisType?: string;
...
export interface SystemDefinition extends Unique, Named {
   ...
   * @implements SY-A1: DEVELOP system logic models that include mission times
   * @example { value: 24, units: "hours", description: "Based on time to establish long-term cooling" }
   */
  missionTime?: {
    /** Numerical value of the mission time */
    value: number;
    
    /** Units of the mission time */
    units: "hours" | "days" | "months";
    
    /** Description or basis for the mission time */
    description?: string;
  };
  
  /**
   * Calculation defaults for system-level parameters used in quantification.
   * Provides default values for basic event calculations when not specified at the event level.
   * @implements SY-A1: DEVELOP system logic models that include mission times
   */
  calculationDefaults?: {
    /** Default calculation method for basic events in this system */
    defaultCalculationMethod: "demand" | "standby" | "operating" | "custom";
    
    /** Default failure rate units for time-based calculations */
    failureRateUnits: "per-hour" | "per-day" | "per-year";
    
    /** Default test interval for standby components (in same units as mission time) */
    defaultTestInterval?: number;
    
    /** Default uncertainty distribution for basic events */
    defaultUncertaintyDistribution?: DistributionType;
    
    /** Default error factor for lognormal distributions */
    defaultErrorFactor?: number;
  };
...
export interface FaultTree extends Unique, Named {
  ...
  quantificationSettings?: {
    ...
    /** Time-based calculation parameters */
    timeBasedParameters?: {
      /** Mission time used in quantification (hours) */
      missionTime: number;
      
      /** Whether to use system-level or basic event-level mission times */
      useEventSpecificMissionTimes?: boolean;
    };
    ...
    uncertaintySettings?: {
      ...
      /** Whether to use event-specific distributions or system defaults */
      useEventSpecificDistributions?: boolean;
      
      /** Default distribution type if not specified at event level */
      defaultDistributionType?: DistributionType;
    };
    
    /** SAPHIRE-specific quantification settings */
    saphireSettings?: {
      /** Cut set update option */
      cutSetUpdateOption?: string;
      
      /** Cut set solution option */
      cutSetSolutionOption?: string;
      
      /** Quantification method code used in SAPHIRE */
      quantificationMethodCode?: string;
    };
  };
  ...
  /**
   * SAPHIRE-specific metadata for import/export compatibility
   */
  saphireMetadata?: {
    /** Whether this is a subtree */
    isSubtree?: boolean;
    
    /** Whether this is an alternate tree */
    isAlternate?: boolean;
    
    /** SAPHIRE project name */
    projectName?: string;
    
    /** Original SAPHIRE file references */
    originalFiles?: {
      ftdFile?: string;
      ftlFile?: string;
      bedFile?: string;
    ...
    /** SAPHIRE-specific flags or settings */
    flags?: string[];
  };
}
...
//==============================================================================
/**
 * @group External Tool Integration
 * @description Interfaces for integration with external PRA tools
 */
//==============================================================================

/**
 * Interface for SAPHIRE import/export functionality.
 * Provides mappings and metadata for converting between OpenPRA and SAPHIRE formats.
 * @group External Tool Integration
 */
export interface SaphireIntegration extends Unique {
  /**
   * SAPHIRE project information
   */
  projectInfo: {
    /** SAPHIRE project name */
    projectName: string;
  ...
  /**
   * Mapping between OpenPRA system references and SAPHIRE system names
   */
  systemMapping: Record<SystemReference, string>;
  
  /**
   * Mapping between OpenPRA basic event IDs and SAPHIRE basic event names
   */
  basicEventMapping: Record<string, string>;
  
  /**
   * Mapping between OpenPRA fault tree IDs and SAPHIRE fault tree names
   */
  faultTreeMapping: Record<string, string>;
  
  /**
   * SAPHIRE file locations for import/export
   */
  fileLocations?: {
    /** Base directory for SAPHIRE files */
  ...
  /**
   * Conversion settings for import/export
   */
  conversionSettings?: {
    /** How to handle mission time conversions */
    missionTimeHandling: "use-system-default" | "use-saphire-value" | "convert-to-hours";
    
    /** How to handle calculation method differences */
    calculationMethodHandling: "preserve-saphire" | "map-to-openpra";
    
    /** How to handle naming conflicts */
    namingConflictResolution: "use-saphire-names" | "use-openpra-ids" | "create-mapping";
    
    /** How to handle distribution type differences */
    distributionTypeMapping?: Record<string, DistributionType>;
    
    /** Custom conversion rules */
    customConversionRules?: Record<string, string>;
  };
}