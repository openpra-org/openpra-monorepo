...
describe('CollabService', () => {
  ...
  describe("updateCell", () => {
    ...
    it("should update cell", async () => {
      ...
      let getFmea = await fmeaService.getFmeaById(fmea.id);
      let rowId = getFmea?.rows[0].id;

      await fmeaService.updateCell(fmea.id,rowId,"test2","value1");
      let res = await fmeaService.updateCell(fmea.id,rowId,"test2","changed value");
      expect(res).toBeDefined();
  ...
  describe("updateDropdownOptions",() => {
    ...
    it("updateDropdownOptions should update dropdownOptions", async () => {
      ...
      await fmeaService.addColumn(fmea.id,addColumnObject2);
      let res = await fmeaService.updateDropdownOptions(fmea.id,"test",[{number:1,description:"changed description"}]);
      expect(res).toBeDefined();
    });
    ...
    it("should not update if column of type string",async () => {
      ...
      await fmeaService.addColumn(fmea.id,addColumnObject2);
      let res = await fmeaService.updateDropdownOptions(fmea.id,"test2",[{number:1,description:"changed description"}]);
  ...
  describe("deleteFmea",() => {
    ...
    it("deleteFmea should delete a fmea", async () => {
      const fmea = await fmeaService.createFmea({title:"test", description:"test"});
      let res = await fmeaService.deleteFmea(fmea.id);
      expect(res).toBeDefined();
      expect(res).toBeTruthy();
    });

    it("should return false if fmea does not exist",async () => {
      let res = await fmeaService.deleteFmea(1);
  ...
  describe("deleteColumn",() => {
    ...
    it("deleteColumn should delete a column", async () => {
      ...
      await fmeaService.addColumn(fmea.id,addColumnObject2);
      let res = await fmeaService.deleteColumn(fmea.id,"test2");
      console.log(res);
    ...
    it("should delete column with rows",async () => {
      ...
      await fmeaService.addColumn(fmea.id,addColumnObject2);
      await fmeaService.addRow(fmea.id);
      let res = await fmeaService.deleteColumn(fmea.id,"test");
      expect(res).toBeDefined();
      expect(res?.columns.length).toEqual(1);
  ...
  describe("deleteRow",() => {
    ...
    it("deleteRow should delete a row", async () => {
      ...
      await fmeaService.addColumn(fmea.id,addColumnObject2);
      await fmeaService.addRow(fmea.id);
      let res = await fmeaService.deleteRow(fmea.id,1);
      expect(res).toBeDefined();
