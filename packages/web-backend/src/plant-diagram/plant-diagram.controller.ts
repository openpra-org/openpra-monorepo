...
export class PlantDiagramController {
  ...
  @Post("/")
  async create(@Body() body: Partial<PlantDiagram>): Promise<PlantDiagram> {
    return this.svc.create(body);
  }

  @Get("/by-model/:modelId")
  async findByModel(@Param("modelId") modelId: string): Promise<PlantDiagram[]> {
    return this.svc.findByModelId(parseId(modelId));
  }

  @Get("/:id")
  async findById(@Param("id") id: string): Promise<PlantDiagram | null> {
    return this.svc.findById(parseId(id));
  }

  @Put("/:id")
  async update(@Param("id") id: string, @Body() body: Partial<PlantDiagram>): Promise<PlantDiagram | null> {
