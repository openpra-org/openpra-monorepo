...
describe("AuthService", () => {
  ...
  describe("loginUser", () => {
    ...
    it("should fail login with incorrect password", async () => {
      ...
      const _response2 = await collabService.createNewUser(user_object);
      try {
        const _result = await authService.loginUser(user_object.username, incorrectPassword);
      } catch (_err: unknown) {
    ...
    it("should fail login with incorrect username", async () => {
      ...
      const _response3 = await collabService.createNewUser(user_object);
      try {
        const _result = await authService.loginUser(incorrectUsername, user_object.password);
      } catch (_err: unknown) {
        expect(_err).toBeInstanceOf(Error);
      }
    });
