...
describe("QuantumReadiness HTTP full pipeline workflow run", () => {
  ...
  it("POST /api/quantum-readiness/workflow/full-pipeline-run creates scaffold and writes all available pipeline artifacts", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "qr-full-pipeline-root-"));

    const candidateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "qr-full-pipeline-candidate-"));
    ...
    const batchRoot = fs.mkdtempSync(path.join(os.tmpdir(), "qr-full-pipeline-batch-"));
