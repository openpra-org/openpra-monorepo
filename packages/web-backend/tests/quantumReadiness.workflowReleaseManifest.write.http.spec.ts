...
describe("QuantumReadiness HTTP workflow release manifest write", () => {
  ...
  it("POST /api/quantum-readiness/release/workflow-manifest/write writes a release manifest artifact", async () => {
    const workflowRunDir = fs.mkdtempSync(path.join(os.tmpdir(), "qr-release-manifest-write-run-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "qr-release-manifest-write-out-"));
