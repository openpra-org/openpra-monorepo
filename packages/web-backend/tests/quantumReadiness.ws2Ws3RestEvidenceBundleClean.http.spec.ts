...
const OUT_BASE = path.join(REPO_ROOT, "_work", "openpra_quantum_ws2_ws3_rest_evidence_bundle_v2");
...
function discoverRealBaselineCaseAnchors(): string[] {
  ...
  for (const root of roots) {
    walkDirectories(root, (dirPath) => {
      ...
      const anchorSegments = anchor.split(path.sep);
      if (anchorSegments.some((segment) => segment.startsWith("_quarantine"))) {
        return;
      }
...
describe("QuantumReadiness WS2 WS3 REST evidence bundle clean v2", () => {
  ...
  it("builds the WS2 WS3 REST evidence bundle", async () => {
    ...
    const runDir = path.join(OUT_BASE, `OPENPRA_WS2_WS3_REST_EVIDENCE_BUNDLE_v2_${stamp}`);
    ...
    const selectedAnchors = anchors.filter((anchor) => !anchor.includes("phase2b_row_9683")).slice(0, 3);
    ...
    writeText(
      memoPath,
      [
        "# OpenPRA WS2 WS3 REST Evidence Memo v2",
        "",
        ...
        "",
        `This clean v2 bundle excludes quarantine paths and excludes row 9683 from the ordinary evidence cohort.
...
Selected real baseline-derived anchors: ${selectedAnchors.length}`,
    ...
    writeJson(summaryJsonPath, {
      artifact_name: "OPENPRA_WS2_WS3_REST_EVIDENCE_BUNDLE_v2",
    ...
    writeJson(manifestPath, {
      artifact_name: "OPENPRA_WS2_WS3_REST_EVIDENCE_MANIFEST_v2",
      generated_at_utc: utcNowIso(),
