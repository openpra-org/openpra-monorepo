...
function analyzeFile(filePath) {
  ...
  const exportDecl =
    /^(export\s+(default\s+)?)(abstract\s+)?(class|interface|type|function|const|let|var|enum)\b/;
...
if (low.length) {
  ...
  for (const r of low)
    console.log(` - ${r.name}: ${r.coverage}% (${r.documented}/${r.total})`);
}

console.log('\nTips to improve:');
console.log(
  '- Ensure all exported classes/functions/types have a TSDoc block (/** ... */) with a one-line summary.',
);
console.log(
  '- Add @param and @returns where applicable; use @public for intended public APIs and mark internals @internal.',
);
console.log(
  '- Re-export intended surface from index.ts to curate public API; keep internals unexported.',
);
console.log(
  '- Avoid documenting specs by excluding *.spec.ts from TypeDoc (done for backend and job-broker).',
);
