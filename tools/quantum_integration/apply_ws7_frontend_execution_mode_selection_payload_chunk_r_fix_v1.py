...
REPLACEMENTS = [
    (
        'return normalized.includes("complete") or normalized == "done" or normalized == "success";',
...
def main() -> None:
    ...
    if not changed:
        print("No execution mode selection payload fixes were needed.")
        return

    print("Applied execution mode selection payload chunk R fix successfully.")
    for item in changed:
        print(item)
