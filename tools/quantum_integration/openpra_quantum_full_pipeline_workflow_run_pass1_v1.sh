...
REPORT_DIR="artifacts/quantum_integration/full_pipeline_workflow_run_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.fullPipelineWorkflowRun.http.spec.ts"
...
echo "==> Writing quantumReadiness.fullPipelineWorkflowRun.http.spec.ts"
...
describe("QuantumReadiness HTTP full pipeline workflow run", () => {
  ...
  it("POST /api/quantum-readiness/workflow/full-pipeline-run creates scaffold and writes all available pipeline artifacts", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-full-pipeline-root-")
    );

    const candidateRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-full-pipeline-candidate-")
    );
    ...
    const batchRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-full-pipeline-batch-")
    );
