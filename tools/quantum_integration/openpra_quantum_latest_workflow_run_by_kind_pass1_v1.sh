...
REPORT_DIR="artifacts/quantum_integration/latest_workflow_run_by_kind_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.latestWorkflowRunByKind.http.spec.ts"
...
echo "==> Writing quantumReadiness.latestWorkflowRunByKind.http.spec.ts"
...
describe("QuantumReadiness HTTP latest workflow run by kind", () => {
  ...
  it("POST /api/quantum-readiness/workflow/latest-run/by-kind returns newest matching workflow kind with inspection summary", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-latest-by-kind-root-")
    );
    ...
    writeJson(
      path.join(runFullNew, "artifacts", "preparation", "openpra_quantum_preparation_bundle_v1.json"),
      { ok: true }
    );
