...
REPORT_DIR="artifacts/quantum_integration/latest_workflow_run_by_target_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.latestWorkflowRunByTarget.http.spec.ts"
...
export class QuantumReadinessService {
  ...
  getLatestWorkflowRunByKind(
    ...
    const listing = this.listWorkflowRuns(rootDir);
    const latest =
      listing.entries.find((entry) => entry.workflowKind === workflowKind) ?? null;
  ...
  getLatestWorkflowRunByTarget(
    ...
    const latest =
      listing.entries.find(
        (entry) => entry.modelId === modelId && entry.subtreeId === subtreeId
      ) ?? null;
...
export class QuantumReadinessController {
  ...
  getLatestWorkflowRunByKind(
    ...
    try {
      return this.quantumReadinessService.getLatestWorkflowRunByKind(
        body.rootDir,
        body.workflowKind
      );
    } catch (error) {
  ...
  getLatestWorkflowRunByTarget(
    ...
    try {
      return this.quantumReadinessService.getLatestWorkflowRunByTarget(
        body.rootDir,
        body.modelId,
        body.subtreeId
      );
...
echo "==> Writing quantumReadiness.latestWorkflowRunByTarget.http.spec.ts"
...
describe("QuantumReadiness HTTP latest workflow run by target", () => {
  ...
  it("POST /api/quantum-readiness/workflow/latest-run/by-target returns newest matching model and subtree with inspection summary", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-latest-by-target-root-")
    );
    ...
    writeJson(
      path.join(runTargetNew, "artifacts", "preparation", "openpra_quantum_preparation_bundle_v1.json"),
      { ok: true }
    );
