...
REPORT_DIR="artifacts/quantum_integration/preparation_workflow_by_id_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.preparationWorkflowById.http.spec.ts"
...
echo "==> Writing quantumReadiness.preparationWorkflowById.http.spec.ts"
...
describe("QuantumReadiness HTTP preparation workflow run by id", () => {
  ...
  it("POST /api/quantum-readiness/workflow/preparation-run/by-id creates scaffold and writes preparation artifacts from stored graph", async () => {
    ...
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-preparation-by-id-root-")
    );
    ...
    expect(
      body.preparationWrite.artifactPaths.every((artifactPath) =>
