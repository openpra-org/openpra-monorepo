...
REPORT_DIR="artifacts/quantum_integration/preparation_workflow_run_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.preparationWorkflowRun.http.spec.ts"
...
echo "==> Writing quantumReadiness.preparationWorkflowRun.http.spec.ts"
...
describe("QuantumReadiness HTTP preparation workflow run", () => {
  ...
  it("POST /api/quantum-readiness/workflow/preparation-run creates scaffold and writes preparation artifacts", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-preparation-workflow-root-")
    );
