...
REPORT_DIR="artifacts/quantum_integration/recovery_artifact_write_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.recoveryArtifacts.write.http.spec.ts"
...
echo "==> Writing quantumReadiness.recoveryArtifacts.write.http.spec.ts"
...
describe("QuantumReadiness HTTP recovery artifacts write", () => {
  ...
  it("POST /api/quantum-readiness/recovery/candidate-dir/write writes a recovery artifact file", async () => {
    const candidateRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-write-candidate-")
    );
    ...
    const outputDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-write-output-")
    );
