...
REPORT_DIR="artifacts/quantum_integration/recovery_batch_rollup_write_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.recoveryBatchRollup.write.http.spec.ts"
...
echo "==> Writing quantumReadiness.recoveryBatchRollup.write.http.spec.ts"
...
describe("QuantumReadiness HTTP recovery batch rollup write", () => {
  ...
  it("POST /api/quantum-readiness/recovery/batch-root/write writes a batch rollup file", async () => {
    const batchRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-batch-root-")
    );
    ...
    const outputDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-batch-rollup-write-")
    );
