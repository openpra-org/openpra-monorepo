...
REPORT_DIR="artifacts/quantum_integration/recovery_batch_workflow_run_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.recoveryBatchWorkflowRun.http.spec.ts"
...
echo "==> Writing quantumReadiness.recoveryBatchWorkflowRun.http.spec.ts"
...
interface RecoveryBatchWorkflowRunHttpResponse {
  workflowRun: {
...
describe("QuantumReadiness HTTP recovery batch workflow run", () => {
  ...
  it("POST /api/quantum-readiness/workflow/recovery-batch-run creates scaffold and writes batch rollup", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-batch-workflow-root-")
    );

    const batchRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-batch-workflow-batch-")
    );
    ...
    const response = await request(app.getHttpServer())
      .post("/api/quantum-readiness/workflow/recovery-batch-run")
      .send({
        rootDir,
        batchRoot,
        modelId: "phase2b_batch_demo",
        subtreeId: "batch_root",
        selectionMode: "package_result_only"
      })
      .expect(200);

    const body = response.body as RecoveryBatchWorkflowRunHttpResponse;
