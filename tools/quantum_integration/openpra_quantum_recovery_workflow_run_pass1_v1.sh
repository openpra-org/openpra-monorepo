...
REPORT_DIR="artifacts/quantum_integration/recovery_workflow_run_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.recoveryWorkflowRun.http.spec.ts"
...
echo "==> Writing quantumReadiness.recoveryWorkflowRun.http.spec.ts"
...
describe("QuantumReadiness HTTP recovery workflow run", () => {
  ...
  it("POST /api/quantum-readiness/workflow/recovery-run creates scaffold and writes recovery artifact", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-workflow-root-")
    );

    const candidateRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-recovery-workflow-candidate-")
    );
