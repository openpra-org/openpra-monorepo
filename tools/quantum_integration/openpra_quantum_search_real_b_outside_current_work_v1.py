...
SCRIPT_VERSION = "openpra-quantum-search-real-b-outside-current-work-v1"

REPO_ROOT = Path("/mnt/storage_array/projects/OPENPRA_DEV_v1/openpra-monorepo")
PROJECT_ROOT = Path("/mnt/storage_array/projects/OPENPRA_DEV_v1")
EXCLUDE_ROOT = REPO_ROOT / "_work"
PROBE_ROOT = REPO_ROOT / "_work" / "openpra_quantum_real_b_search_outside_work_probe_v1"
OUT_DIR = REPO_ROOT / "_work" / "openpra_quantum_real_b_search_outside_work_v1"
...
def main() -> None:
    ...
    export_files = sorted(PROJECT_ROOT.rglob("*_clqubo_export.json"))
    rows: list[CaseRow] = []

    filtered = [p for p in export_files if not str(p).startswith(str(EXCLUDE_ROOT))]
    for idx, export_path in enumerate(filtered, start=1):
    ...
    summary_path = OUT_DIR / "openpra_quantum_real_b_search_outside_work_summary_v1.json"
    write_json(summary_path, summary)

    memo_lines = [
        "OpenPRA Quantum Real B Search Outside Current Work Memo v1",
        "",
    ...
    if real_b_rows:
        ...
        memo_lines.extend([
            "Interpretation",
            "",
            "No real B candidates were found outside the current monorepo _work lane in this pass.",
            "That means a different source lane or a different upstream generation path would be required to demonstrate real B.",
        ])

    memo_lines.extend(["", f"Summary JSON: {summary_path}", ""])

    memo_path = OUT_DIR / "OPENPRA_QUANTUM_REAL_B_SEARCH_OUTSIDE_WORK_MEMO_v1.txt"
