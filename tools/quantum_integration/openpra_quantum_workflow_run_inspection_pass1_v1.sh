...
REPORT_DIR="artifacts/quantum_integration/workflow_run_inspection_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.workflowRunInspection.http.spec.ts"
...
echo "==> Writing quantumReadiness.workflowRunInspection.http.spec.ts"
...
describe("QuantumReadiness HTTP workflow run inspection", () => {
  ...
  it("POST /api/quantum-readiness/workflow/inspect-run returns a summary of workflow outputs", async () => {
    const workflowRunDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-workflow-inspect-")
    );
    ...
    writeJson(
      path.join(preparationDir, "openpra_quantum_preparation_bundle_v1.json"),
      { ok: true }
    );
    writeJson(
      path.join(preparationDir, "openpra_quantum_preparation_artifact_alpha.json"),
      { ok: true }
    );
    writeJson(
      path.join(executionDir, "openpra_quantum_execution_artifact_v1.json"),
      { ok: true }
    );
    writeJson(
      path.join(executionDir, "openpra_quantum_execution_provenance_manifest_v1.json"),
      { ok: true }
    );
    writeJson(
      path.join(recoveryDir, "openpra_quantum_recovery_artifact_v1.json"),
      { ok: true }
    );
    writeJson(
      path.join(batchDir, "openpra_quantum_recovery_batch_rollup_v1.json"),
      { ok: true }
    );
    ...
    expect(body.manifestPath).toContain(
      "openpra_quantum_workflow_run_manifest_v1.json"
    );
