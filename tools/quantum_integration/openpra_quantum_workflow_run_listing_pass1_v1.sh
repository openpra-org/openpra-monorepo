...
REPORT_DIR="artifacts/quantum_integration/workflow_run_listing_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.workflowRunListing.http.spec.ts"
...
echo "==> Writing quantumReadiness.workflowRunListing.http.spec.ts"
...
describe("QuantumReadiness HTTP workflow run listing", () => {
  ...
  it("POST /api/quantum-readiness/workflow/list-runs returns workflow runs sorted newest first", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-workflow-list-root-")
    );
