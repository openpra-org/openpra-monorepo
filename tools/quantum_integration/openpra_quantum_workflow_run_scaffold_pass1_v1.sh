...
REPORT_DIR="artifacts/quantum_integration/workflow_run_scaffold_pass1_${UTC_NOW}"
mkdir -p "${REPORT_DIR}"

INDEX_PATH="packages/quantum-readiness/src/lib/index.ts"
RUN_SCAFFOLD_PATH="packages/quantum-readiness/src/lib/openpra-quantum-workflow-run-scaffold.ts"
RUN_SCAFFOLD_SPEC_PATH="packages/quantum-readiness/src/lib/openpra-quantum-workflow-run-scaffold.spec.ts"
...
HTTP_SPEC_PATH="packages/web-backend/tests/quantumReadiness.workflowRunScaffold.http.spec.ts"

echo "==> Writing openpra-quantum-workflow-run-scaffold.ts"
cat > "${RUN_SCAFFOLD_PATH}" <<'EOF'
...
echo "==> Writing openpra-quantum-workflow-run-scaffold.spec.ts"
cat > "${RUN_SCAFFOLD_SPEC_PATH}" <<'EOF'
import fs from "node:fs";
...
echo "==> Writing quantumReadiness.workflowRunScaffold.http.spec.ts"
...
describe("QuantumReadiness HTTP workflow run scaffold", () => {
  ...
  it("POST /api/quantum-readiness/workflow/run-scaffold creates a workflow run directory tree", async () => {
    const rootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "qr-workflow-run-root-")
    );
