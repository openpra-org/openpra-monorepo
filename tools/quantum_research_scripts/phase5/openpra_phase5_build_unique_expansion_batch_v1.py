...
DEFAULT_PACKAGE_RUN = ROOT / "_work" / "openpra_phase4_reference_artifact_packages_v1" / "20260410_143658Z"
DEFAULT_ACCEPTANCE_ROOT = ROOT / "_work" / "openpra_phase5_final_acceptance_bundle_v1"
DEFAULT_OUTPUT_ROOT = ROOT / "_work" / "openpra_phase5_unique_expansion_batch_v1"
SCRIPT_VERSION = "openpra-phase5-build-unique-expansion-batch-v1"
...
def parse_package_metadata(path: Path) -> Dict[str, Any]:
    ...
    normalized["model_id"] = obj.get("model_id") or obj.get("modelId")
    normalized["candidate_root_node_id"] = obj.get("candidate_root_node_id") or obj.get("candidateRootNodeId")
    normalized["topology_class"] = obj.get("topology_class") or obj.get("topologyClass")
    normalized["basic_event_count"] = obj.get("basic_event_count") or obj.get("basicEventCount")
    normalized["required_qubits"] = obj.get("required_qubits") or obj.get("requiredQubits")

    basic_ids = obj.get("ordered_basic_event_ids") or obj.get("orderedBasicEventIds") or obj.get("basic_event_ids") or obj.get("basicEventIds")
    if basic_ids is None:
        basic_ids = []
    normalized["basic_event_ids"] = list(basic_ids)
...
def parse_classical_reference(path: Path) -> Tuple[int, List[List[str]]]:
    ...
    count = frozen.get("minimalCutSetCount") or frozen.get("minimal_cut_set_count") or 0
    event_sets = frozen.get("basicEventIdSets") or frozen.get("basic_event_id_sets") or []
    normalized_sets = [sorted(list(s)) for s in event_sets]
    normalized_sets.sort()
    return int(count), normalized_sets
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Audit structural uniqueness and build the next unique executed-case expansion batch."
    )
    ap.add_argument("--batch-root", default=str(DEFAULT_BATCH_ROOT))
    ap.add_argument("--package-run", default=str(DEFAULT_PACKAGE_RUN))
    ...
    package_run = Path(args.package_run).resolve()
    ...
    package_case_dirs = find_case_dirs(package_run)
    if not package_case_dirs:
        raise SystemExit(f"No package case directories found under: {package_run}")

    all_package_records = [load_case_record(d) for d in package_case_dirs]
    ...
    selected = unique_representatives[:target_new_count]
    ...
    summary = {
        ...
        "package_case_count": len(all_package_records),
    ...
    readme_lines = [
        ...
        f"package_case_count: {len(all_package_records)}",
